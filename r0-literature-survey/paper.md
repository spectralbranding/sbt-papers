...
This paper surveys geometric and topological methods applied to brand perception across ten intellectual traditions: multidimensional scaling, characteristics-space economics, conceptual spaces, non-Euclidean perceptual geometry, information geometry, individual-differences modeling, topological data analysis, quantum cognition, opinion dynamics, and non-ergodicity research. Against five criteria stipulated here, a compound gap persists: no surveyed framework combines high-dimensional geometric structure, a generative signal mechanism, observer heterogeneity, and non-ergodic temporal dynamics into an integrated theory of brand perception. Multidimensional scaling operates in two to three dimensions and offers no generative mechanism. Conceptual spaces provide geometric foundations for cognition but lack temporal dynamics and observer-specific processing. Non-Euclidean perceptual geometry demonstrates that Euclidean distance is insufficient for modeling perception, yet this insight has not penetrated brand theory. Non-ergodicity has been formalized in psychology, decision science, and evolutionary biology, but has not been applied to brand perception outside the present programme. Six open problems with formal mathematical statements — concerning formal metrics on brand space, projection bounds, concentration of measure, positioning capacity, specification impossibility, and diffusion dynamics on perceptual manifolds — constitute a research agenda for mathematical brand theory; companion papers [@zharnikov-2026-brand-space-geometry-formal-metric; @zharnikov-2026-spectral-metamerism-brand-perception-projection; @zharnikov-2026-cohort-boundaries-high-dimensional-perception; @zharnikov-2026-many-brands-can-market-hold; @zharnikov-2026-specification-impossibility-organizational-design-high; @zharnikov-2026-non-ergodic-brand-perception-diffusion] resolve each.

**Keywords**: brand perception, geometric methods, multidimensional scaling, conceptual spaces, non-ergodicity, high-dimensional geometry
...
**Figure 1.** Compound-gap Venn schematic. Three literature gaps (geometric, cohort-aware, observer-mediated) identified by this survey have not been addressed simultaneously by any prior framework. SBT is positioned as a candidate at the intersection. Each gap corresponds to a set of open problems in the research agenda.
...
### Dimensionality: The Two-to-Three Convention
...
This raises a fundamental issue that the MDS literature has not resolved. If the underlying perceptual space is genuinely higher-dimensional, low-dimensional MDS solutions are not merely approximate---they are systematically distorted projections. The Johnson-Lindenstrauss lemma [@johnson-1984-extensions-lipschitz-mappings] provides formal bounds on the distortion introduced by dimensionality reduction. The lemma is not foreign to marketing: Chiong and Shum [-@chiong-2019-random-projection-discrete-choice] justify a random-projection estimator for discrete-choice models with high-dimensional choice sets by exactly this result, and validate it on supermarket scanner data. What has not been done is to turn the bounds on the dimensionality-selection decision itself---to ask what a two-dimensional perceptual map provably distorts when the space beneath it is higher-dimensional. The companion paper identified in the research agenda does this, and it postdates this survey.

### No Generative Mechanism
...
### Bayesian and Probabilistic Extensions
...
## Characteristics Space Models

### Lancaster's New Consumer Theory
...
**Figure 2.** Cluster citation map. Arrows show which of the five literature clusters (left) contribute intellectual tools to each of the six open problems in the research agenda (right). Problems 1 and 4 draw on multiple clusters; Problem 6 is driven by the dynamics traditions alone.

## Conceptual Spaces

### Gardenfors's Framework
...
### Applications to Brand and Product Perception
...
*Euclidean assumption*. Gardenfors assumes that quality dimensions are equipped with Euclidean or at least metric structure. This is a strong assumption given the evidence that human perceptual space is non-Euclidean.
...
## Non-Euclidean Perceptual Geometry
...
### The Tversky Asymmetry Critique
...
## Individual Differences and Dimension Weighting

### The INDSCAL Tradition
...
### The Missing Step to Formal Metric Spaces
...
## Topological Data Analysis in Marketing
...
### Hilbert Space Formalism for Decision-Making
...
### Applications to Brand Perception
...
However, quantum cognition has not been applied to brand perception formation as a temporal, cumulative process. The limit is not that the formalism lacks dynamics. It has an explicit dynamical branch, in which the state evolves as a wave across levels of support and a decision is constructed from the superposition when a response is requested, and that branch has been compared head to head against Markov dynamics on empirical decision data [@busemeyer-2020-markov-versus-quantum-dynamical]. The limit is the timescale those dynamics describe: evolution within a single deliberation, terminating at the moment of choice. What is not addressed is how brand perceptions accumulate across many separated encounters---how signals decay between them, and how past encounters create priors that shape later readings.
...
## Opinion Dynamics and Multi-Agent Models
...
### Gaps Relative to Brand Perception
...
## Non-Ergodicity Beyond Economics

### Peters's Ergodicity Economics
...
### Non-Ergodicity in Biology and Ecology
...
Despite the extensive application of non-ergodicity to psychology, decision science, ecology, and organizational theory, we are not aware of work outside the present research programme applying Peters's formalism to brand perception, customer loyalty, or marketing metrics. The qualification is necessary: the companion paper identified at the end of this section does apply it, and it postdates this survey.

One boundary is worth marking, because the word appears in the marketing literature without the formalism behind it. Clempner and Poznyak [-@clempner-2014-simple-computing-customer-lifetime] compute customer lifetime value under a Markov decision process and include an explicit ergodicity-verification step. That is ergodicity in the standard stochastic-process sense---whether the chain reaches a recurrent steady state---not the divergence between time-average and ensemble-average growth under multiplicative dynamics. The distinction is the whole content of Peters's result, and models of the former kind are common in marketing while models of the latter kind are what we find absent.
...
## Synthesis: The Geometric Brand Theory Gap
...
| Quantum Cognition (Busemeyer) | Finite Hilbert space | Inner product | State vectors | Unitary evolution (within-deliberation) | Measurement collapse | Moderate (decision experiments) |
...
*Notes*: SBT temporal dynamics and generative mechanism entries reflect proposed formalizations in companion papers; empirical validation is illustrative pending independent replication. "Proposed (2026X)" indicates the companion paper developing the formal property cited. All six criteria are formally defined in the classification matrix.

### The Compound Gap

No existing tradition satisfies all five of the compound-gap criteria. Criterion 6 of the classification matrix---empirical validation---is reported separately in Tables 1a-1b and is deliberately not part of the gap statement, because a framework proposing a research agenda cannot claim an advantage on it. Before reading the shortfall as a finding about the traditions, it is worth being explicit about where the criteria came from: they are this survey's own, stipulated from the structural absences catalogued above, and they name the properties a candidate framework is being built to supply. A tradition that was never trying to supply all six is not thereby deficient, and the joint shortfall is a property of the criterion set as much as of the literature. What the classification matrix contributes is to state that set explicitly and place each tradition against it. Each tradition excels on one or two criteria while leaving others unaddressed:
...
- Non-Euclidean perceptual geometry establishes that Euclidean distance is inadequate, but as a claim about *perceptual* structure it has been developed only for sensory perception (color, spatial vision), not for multi-attribute brand perception. Non-Euclidean geometry itself does reach consumer-facing representations---hyperbolic embeddings are used to model hierarchical contextual structure in recommender systems [@bauman-2025-hypercars-hyperbolic-embeddings]---but as representation learning chosen for its capacity to embed hierarchies, not as a finding about the geometry of perception.
...
The shortfall is therefore not in any one ingredient but in the combination, as Table 2 summarizes across the surveyed traditions. No existing framework integrates high-dimensional geometric structure with a formal metric, observer-specific processing, non-ergodic temporal dynamics, and a generative signal mechanism — a statement about where the surveyed literature sits relative to the criteria stipulated here, not a measurement of the literature against a neutral standard.

**Table 2.** Positioning of Surveyed Traditions Against the Five Compound-Gap Criteria.
...
*Notes*: Cell entries are read against the five compound-gap criteria defined in the classification matrix (criteria 1, 2, 3, 4, 5; criterion 6 — empirical validation — is reported separately in Tables 1a–1b). "yes" = the tradition formally satisfies the criterion; "partial" = the tradition addresses the criterion in a restricted form (e.g., low-dimensional only, within-deliberation only, or without proved properties); "—" = the criterion is not addressed within the tradition; "proposed" = SBT companion papers [@zharnikov-2026-brand-space-geometry-formal-metric; @zharnikov-2026-spectral-metamerism-brand-perception-projection; @zharnikov-2026-cohort-boundaries-high-dimensional-perception; @zharnikov-2026-many-brands-can-market-hold; @zharnikov-2026-specification-impossibility-organizational-design-high; @zharnikov-2026-non-ergodic-brand-perception-diffusion] develop the formal property cited. The central five-way intersection — a single tradition with "yes" entries across all five columns — is unoccupied by construction: the five criteria are the author's own, derived from the structural absences catalogued above and matching the design commitments of the candidate framework the survey positions, so the table locates each tradition relative to those commitments and is not a neutral evaluation of the traditions on their own terms. SBT's own row is entered as "proposed", and its cells become "yes" only upon the resolution of the six open problems in the research agenda.

### SBT as a Candidate Framework
...
The Generalized Context Model [@nosofsky-1986-attention-similarity-identificationcategorization] and Representational Similarity Analysis [@kriegeskorte-2008-representational-similarity-analysis] are discussed as precursors and parallels, respectively, within the observer-heterogeneity tradition. Neither satisfies the compound gap criteria because neither integrates non-ergodic temporal dynamics with a generative signal mechanism.
...
Four additional boundary conditions constrain the survey's scope and should be kept in view when reading the open-problem formulations in the research agenda.
...
**Context.** The companion OrgSchema Theory (OST) framework [@zharnikov-2026-organizational-schema-theory-test-driven] uses an 8 x 6 activation matrix, creating a 48-dimensional specification space. An earlier statement of this problem motivated it by the volume of the unit ball in 48 dimensions, read as showing that any organization "occupies" an infinitesimal fraction of the specification hypercube. That motivation is withdrawn. The ratio of ball volume to enclosing cube volume in $\mathbb{R}^{48}$ is fixed by an arbitrary choice of radius scale: rescaling the units rescales the fraction without changing anything about the organization, so the quantity carries no organizational content. The problem's content lies instead in the resolution-indexed count below, where $\epsilon$ is a stated measurement resolution rather than a free scale — it is against $(1/\epsilon)^{48}$ distinguishable specifications, not against a unit-dependent volume, that realizable organizational configurations must be compared.
...
Table 3 consolidates the companion-paper roadmap, pairing each open problem with the paper that develops it and the core mathematical tool involved.
