#!/usr/bin/env python3
"""Q1: Compounding x Structured Specification Experiment.

Pre-registered replication of Exp D v2 (compounding_format_v2). Tests whether
constraint framing ("distribute attention equally across all eight dimensions")
reduces dimensional collapse at step_3 of a multi-turn agentic pipeline, and
whether the 42% reduction observed in Exp D v2 (d = -.983) replicates in an
independent randomization.

DESIGN

Three conditions x three pipeline stages x control:
  - baseline:     standard system prompt, no additional framing
  - information:  Brand Function JSON prepended to system prompt at every step
  - constraint:   structured attention directive prepended to system prompt at
                  every step (no brand-specific scores)

Pipeline stages (single multi-turn conversation per run):
  - step_1:  Retrieval -- recommend 5 brands in category (free text, no parsing)
...
MODELS: claude-haiku-4-5, gpt-4o-mini, gemini-2.5-flash, deepseek-chat,
        grok-4-1-fast-non-reasoning (cloud only; gemma4 excluded: 0% parse rate)
REPS:   2 per cell
...
HYPOTHESES (pre-registered)
  H_Q1a: DCI(step_3, constraint) < DCI(step_3, baseline)  -- d < -.80
         -- constraint reduces final collapse by >= 30%
  H_Q1b: DCI(step_3, constraint) < DCI(step_3, information)  -- d < -.50
         -- constraint outperforms information at final stage
  H_Q1c: DCI(control, constraint) < DCI(control, baseline)
         -- constraint reduces single-step collapse (floor effect)

OUTPUT

  L3_sessions/exp_q1_compounding_spec.jsonl  -- raw session log (appended)

USAGE

    uv run python exp_q1_compounding_spec.py --dry-run  # no API calls
    uv run python exp_q1_compounding_spec.py --smoke    # 1 brand x 1 model x 3 conditions
    uv run python exp_q1_compounding_spec.py --live     # full 600 calls
...
# Brand Function JSON files (relative to script's grandparent directory)
BRAND_FUNCTION_DIR = (
...
# Constraint text: procedural directive without brand-specific scores
CONSTRAINT_TEXT = (
    "When evaluating brands, you must distribute your attention equally across all "
    "eight brand perception dimensions: Semiotic, Narrative, Ideological, "
    "Experiential, Social, Economic, Cultural, and Temporal. Do not concentrate "
    "your evaluation on price or functional features alone. Each dimension should "
    "receive approximately equal consideration in your assessment."
)


# ---------------------------------------------------------------------------
# Models (cloud only; gemma4 excluded -- 0% parse rate in compounding_format_v1)
# ---------------------------------------------------------------------------
...
def build_information_system_prompt(brand: str, base_system: str) -> str:
    """Prepend Brand Function spec to base system prompt (information condition).
...
def build_constraint_system_prompt(base_system: str) -> str:
    """Prepend attention-balancing constraint to base system prompt.

    No brand-specific scores are injected. The constraint is a procedural
    directive testing whether behavioral instruction alone reduces collapse.
    """
...
def build_system_prompt(brand: str, base_system: str, bf_condition: str) -> str:
    ...
    Args:
        ...
        base_system: Base system prompt string.
    ...
    if bf_condition == "information":
        return build_information_system_prompt(brand, base_system)
...
def compute_dci(weights: dict[str, float]) -> float:
    """DCI = mean(|w_i - 12.5|) for all 8 dimensions.

    DCI = 0 indicates perfect uniform distribution.
    Higher values indicate greater dimensional collapse.
    """
    total = sum(weights.values())
    if total == 0:
        return 0.0
    # Renormalize to sum to 100 before computing DCI
    factor = 100.0 / total
    normalized = {k: v * factor for k, v in weights.items()}
    return sum(abs(normalized[d] - 12.5) for d in DIMENSIONS) / len(DIMENSIONS)
...
def _make_record(
    ...
    Includes all required schema fields (timestamp, model, prompt_type, prompt)
    and all recommended fields from EXPERIMENT_DATA_STANDARD.md.
    """
    cfg = MODELS[model_name]
    return {
        # Required schema fields
        "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(),
        "model": model_name,  # short canonical name (matches models.yaml)
        "prompt_type": "compounding_spec",
        "prompt": user_prompt,
        # Recommended fields
        ...
        "experiment": "exp_q1_compounding_spec",
        ...
        "dim_order": dim_order,
        # Weight fields (filled after parsing)
        "parsed_weights": None,
        "weights_valid": False,
        "weight_sum_raw": 0,
        "dci": None,
        ...
        # Step-tracking fields (filled during pipeline)
        "step_1_brands": None,
        "step_1_raw": None,
        "competitor": None,
        "step_2_raw": None,
...
def _print_progress(record: dict, label: str) -> None:
    ...
    if record["step"] == 1:
        ...
        dci_str = f"DCI={record['dci']:.3f}" if record["dci"] is not None else ""
        status = f"OK {dci_str}"
    else:
        status = "FAIL"
...
def _dry_run_record(record: dict, label: str) -> None:
    ...
    # Provide fake uniform weights so downstream parsing does not fail
    fake_weights = {d: 12.5 for d in DIMENSIONS}
    record["parsed_weights"] = fake_weights
    record["parsed"] = fake_weights
...
def run_pipeline(
    ...
    The system prompt is selected once for the condition and held constant
    across all three steps (persistent context per conversation).
    ...
    if dry_run:
        ...
        try:
            ...
            record1["response"] = raw1
            record1["latency_ms"] = meta1.get("response_time_ms")
            record1["tokens_in"] = meta1.get("token_count_input")
            record1["tokens_out"] = meta1.get("token_count_output")
            record1["response_time_ms"] = meta1.get("response_time_ms", 0)
            record1["token_count_input"] = meta1.get("token_count_input", 0)
            record1["token_count_output"] = meta1.get("token_count_output", 0)
            record1["api_cost_usd"] = meta1.get("api_cost_usd", 0.0)
            ...
            record1["response"] = err_msg
            record1["error"] = err_msg
            step1_brands = []
    ...
    record2["step_1_raw"] = record1["response"]
    ...
    if dry_run:
        ...
        try:
            ...
            record2["response"] = raw2
            record2["latency_ms"] = meta2.get("response_time_ms")
            record2["tokens_in"] = meta2.get("token_count_input")
            record2["tokens_out"] = meta2.get("token_count_output")
            record2["response_time_ms"] = meta2.get("response_time_ms", 0)
            record2["token_count_input"] = meta2.get("token_count_input", 0)
            record2["token_count_output"] = meta2.get("token_count_output", 0)
            record2["api_cost_usd"] = meta2.get("api_cost_usd", 0.0)
            ...
            if wa:
                ...
                record2["dci"] = compute_dci(wa)
            else:
                record2["weights_valid"] = False
            history.append({"role": "assistant", "content": raw2})
        except Exception as e:
            err_msg = f"ERROR: {type(e).__name__}: {e}"
            record2["response"] = err_msg
            record2["error"] = err_msg
            raw2 = ""
            history.append({"role": "assistant", "content": err_msg})
    ...
    record3["step_1_raw"] = record1["response"]
    record3["step_2_raw"] = record2["response"]
    ...
    if dry_run:
        ...
        try:
            ...
            record3["response"] = raw3
            record3["latency_ms"] = meta3.get("response_time_ms")
            record3["tokens_in"] = meta3.get("token_count_input")
            record3["tokens_out"] = meta3.get("token_count_output")
            record3["response_time_ms"] = meta3.get("response_time_ms", 0)
            record3["token_count_input"] = meta3.get("token_count_input", 0)
            record3["token_count_output"] = meta3.get("token_count_output", 0)
            record3["api_cost_usd"] = meta3.get("api_cost_usd", 0.0)
            recommended, weights, reason = parse_step3_response(raw3)
            if weights:
                record3["parsed_weights"] = weights
                record3["parsed"] = weights
                wsum = sum(weights.values())
                record3["weight_sum_raw"] = wsum
                record3["weights_valid"] = abs(wsum - 100) <= 5
                record3["dci"] = compute_dci(weights)
            else:
            ...
            err_msg = f"ERROR: {type(e).__name__}: {e}"
            record3["response"] = err_msg
            record3["error"] = err_msg
...
def run_control(
    ...
    """Run a single-step PRISM-B control evaluation (no conversation context).

    In the 'information' condition the Brand Function spec is prepended.
    In the 'constraint' condition the attention-balancing directive is prepended.
    ...
    if dry_run:
        ...
        try:
            ...
            record["response"] = raw
            record["latency_ms"] = meta.get("response_time_ms")
            record["tokens_in"] = meta.get("token_count_input")
            record["tokens_out"] = meta.get("token_count_output")
            record["response_time_ms"] = meta.get("response_time_ms", 0)
            record["token_count_input"] = meta.get("token_count_input", 0)
            record["token_count_output"] = meta.get("token_count_output", 0)
            record["api_cost_usd"] = meta.get("api_cost_usd", 0.0)
            ...
            if weights:
                ...
                record["dci"] = compute_dci(weights)
        except Exception as e:
            err_msg = f"ERROR: {type(e).__name__}: {e}"
            record["response"] = err_msg
...
def run_experiment(mode: str = "live", dry_run: bool = False) -> None:
    """Execute the Q1 compounding x specification experiment."""
    output_dir = Path(__file__).resolve().parent.parent / "L3_sessions"
    output_dir.mkdir(parents=True, exist_ok=True)
    jsonl_path = output_dir / "exp_q1_compounding_spec.jsonl"
    ...
    print(f"\nQ1: Compounding x Specification Experiment{dry_tag}")
    ...
    print(f"\n{'='*60}")
    print(f"Q1 Compounding x Specification -- COMPLETE{dry_tag}")
    print(f"{'='*60}")
    ...
    for bf_cond in ("baseline", "information", "constraint"):
        ...
        dci_values = [r["dci"] for r in cond_recs if r["dci"] is not None]
        mean_dci = sum(dci_values) / len(dci_values) if dci_values else 0.0
        print(
            f"  {bf_cond}: {len(cond_recs)} records, "
            f"{valid}/{weight_bear} weight-bearing valid, "
            f"mean DCI={mean_dci:.3f}"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Q1: Compounding x Specification (pre-registered replication)"
    )
    ...
    mode_group.add_argument(
        "--smoke",
        action="store_true",
        help="Smoke test: 1 brand x 1 model x 3 conditions (~12 calls)",
    )
    mode_group.add_argument(
        "--live",
        action="store_true",
        help="Full run: 5 brands x 5 models x 2 reps x 3 conditions (~600 calls)",
    )
    args = parser.parse_args()

    if args.dry_run:
        mode = "live"  # use full brand/model set to validate all code paths
        dry_run = True
    elif args.smoke:
        mode = "smoke"
        dry_run = False
    ...
    print(f"Q1 Compounding x Specification ({mode_label})")
