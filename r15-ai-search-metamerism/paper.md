...
Working Paper v3.2.0 – June 2026 (revised June 2026)
...
**PRISM-B: The Measurement Instrument.** The experiment uses PRISM-B (Perception Response Instrument for Structured Measurement --- Brand), an open-source instrument for measuring multi-dimensional LLM brand perception. PRISM-B implements a five-layer scaffold (PL0: pre-registered protocol; PL1: model configuration; PL2: versioned prompt templates; PL3: per-call JSONL session logs; PL4: analysis scripts) ensuring full reproducibility --- any researcher can fork the repository, substitute brand pairs, and run the pipeline against any LLM with a chat-completion API. PRISM-B was used across all ten runs without structural modification. The full scaffold specification is in the Web Appendix; the PRISM-B pipeline and analysis scripts are at `github.com/spectralbranding/sbt-papers/r15-ai-search-metamerism/experiment/`, and the full per-call data and session logs are archived as the dataset of record on HuggingFace at https://doi.org/10.57967/hf/8284.
