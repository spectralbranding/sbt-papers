...
|:-----------|:-------------------|:-----------|:----------|:-------|
| Aaker BPS [-@aaker-1997-dimensions-brand-personality] | Personality traits (5) | Factor analysis | Human only | 1--5 Likert |
| Brakus BXS [-@brakus-2009-brand-experience-what] | Experiential reactions (4) | Factor analysis | Human only | 1--7 Likert |
| Yoo-Donthu CBBE [-@yoo-2001-developing-validating-multidimensional] | Brand equity outcomes (3) | Factor analysis | Human only | 1--5 Likert |
...
**PL0: Specification.** The pre-registered protocol layer. Contains hypotheses, stopping rules, brand/observer selection criteria, planned analyses, and alpha allocation. PL0 is written and frozen before any data collection begins. Its purpose is to enforce the confirmatory-exploratory boundary [@nosek-2018-the-preregistration-revolution]: analyses specified at PL0 are confirmatory; all others are exploratory regardless of their statistical significance.

**PL1: Configuration.** The technical infrastructure layer. Contains model backend definitions (API endpoints, authentication, model identifiers), observer cohort definitions (for human administration), response format parameters (scale points, output schema), temperature and sampling parameters, and retry/timeout policies. PL1 is declarative: it specifies what is measured and how the measurement infrastructure is configured, not the content of what is asked. In the AI observer context, PL1 is implemented as a YAML configuration file; for human administration, PL1 specifies the survey platform, randomization procedure, and demographic collection protocol.

**PL2: Prompts.** The item layer. Contains the exact wording of each measurement prompt, the structured output schema (JSON for AI observers, Likert display for human respondents), version identifiers, and language variants. PL2 is the only layer that differs across PRISM variants: PRISM-B has brand perception items, PRISM-M has metamerism discrimination items, PRISM-T has temporal comparison items. The prompt template is versioned so that cross-study comparisons can verify item equivalence.

**PL3: Sessions.** The raw data layer. Every administration produces a session record containing the full prompt sent, the raw response received, the parsed structured output, observer metadata (model identifier for AI, demographic profile for human respondents), a timestamp, and a response validity flag. PL3 is append-only and immutable: no session record is modified or deleted after collection. This ensures auditability and permits re-analysis with updated scoring algorithms. For AI observers, PL3 is implemented as a JSONL file (one JSON object per API call). For human respondents, PL3 is the equivalent raw response export from the survey platform.
...
*Separation of concerns.* Each layer has a single responsibility. Changing the response format (PL1) does not require changing the items (PL2). Adding a new hypothesis (PL0) does not require modifying the data collection infrastructure (PL1/PL3). This separation permits controlled experimentation on each layer independently — for example, testing whether a 1--5 ordinal format outperforms a 1--7 format while holding items and analysis constant.

*Reproducibility by construction.* The combination of PL0 (frozen protocol), PL1 (declarative configuration), PL2 (versioned prompts), and PL3 (immutable session data) ensures that any PRISM study can be reproduced by forking the instrument repository and re-executing the PL1 configuration against the same observer population. For AI observers, exact reproduction requires specifying model version identifiers; for human respondents, it requires equivalent sampling procedures.
...
The five-point format is not arbitrary. It is the empirically observed minimum-distortion operating point for AI observers, derived from rate-distortion analysis of AI brand perception encoders. Zharnikov [-@zharnikov-2026-empirical-rate-distortion-curve-ai] tested five response formats spanning 3 to 26 bits of information capacity across 17 LLM architectures and five canonical reference brands. The 1--5 ordinal format (approximately 19 bits, computed as $\log_2(5^8) = 18.6$ bits for eight items) achieved the lowest distortion from canonical brand profiles, outperforming both the higher-rate 100-point allocation format (approximately 26 bits) and lower-rate formats. The effect is large: paired *t*(16) = 11.92, *p* < .001, Cohen's *d*~z~ = 2.89 for the 100-point versus 1--5 comparison. All 17 architectures from distinct training lineages exhibited this pattern without exception.

The mechanism is encoder bias suppression: when the response space is unconstrained (100 points across eight dimensions), LLM encoders express idiosyncratic training-corpus biases that increase distortion. The bounded five-point format constrains the output alphabet sufficiently to suppress these biases while preserving enough resolution to capture meaningful dimensional differentiation. This result is a concrete instance of the rate-distortion prediction formalized by Sims [-@sims-2016-ratedistortion-theory-human], who demonstrated that human perceptual capacity limits produce optimal categorical encodings at intermediate rates — precisely the pattern observed here for AI encoders. The parallel extends to human survey methodology, where Krosnick [-@krosnick-1991-response-strategies-coping] documented satisficing under cognitively demanding formats and Schwarz [-@schwarz-1999-selfreports-how-questions] showed that response scales shape answers by providing implicit frames of reference. The convergence of information theory [@sims-2016-ratedistortion-theory-human], AI encoding [@zharnikov-2026-empirical-rate-distortion-curve-ai], and survey design [@krosnick-1991-response-strategies-coping; @schwarz-1999-selfreports-how-questions] on the same conclusion strengthens the format choice beyond any single line of evidence.
...
1. Select models and configure PL1 (endpoints, parameters, temperature = .7).
...
5. Record each valid response as a PL3 session record.

**For human respondents:**

1. Configure the survey platform with the eight items (Table 2) and the 1--5 response scale.
...
5. Record all responses as PL3 session records.
...
Positive deviations indicate dimensional inflation (the observer over-weights this dimension relative to uniform); negative deviations indicate dimensional suppression. Reporting per-dimension deviations alongside aggregate metrics (DCI, cosine) prevents the aggregate from masking meaningful dimensional heterogeneity.
...
The observation model [@zharnikov-2026y-brand-triangulation] treats each observer cohort *k* as providing:
...
PRISM-B is one member of a family of instruments sharing the PL0--PL4 scaffold. Each variant inherits the architecture and substitutes domain-specific items at PL2 and scoring algorithms at PL4. Since this specification was first issued, four sibling instruments have been specified as standalone pre-registered measurement campaigns, each cited to its own record; a fifth is planned. A recurring design principle unifies them: a measured gap is reported only when it clears the instrument's own empirically estimated floor, so the family reports sub-resolution rather than manufacturing findings.

**PRISM-M (Metamerism)** [@zharnikov-2026az-prism-m-metamerism] measures whether two entities are observationally equivalent under a compressing readout. It is built on a dual-floor procedure: a pair counts as a measured metamer only when the full multi-dimensional readout resolves it beyond the instrument's operator noise floor while an aggregate readout (scalar score, ranking, or binary pick), with its own separately measured floor, does not. PRISM-M operationalizes the spectral metamerism concept [@zharnikov-2026-spectral-metamerism-brand-perception-projection], with the construct empirically grounded in AI-native brand identity evolution [@zharnikov-2026-ai-native-brand-identity-from]; its pre-registered campaign demonstrates the discipline end-to-end, reporting wholesale sub-resolution when a discordant operator inflates every floor rather than forcing a positive result.

**PRISM-T (Temporal)** [@zharnikov-2026ba-prism-t-version-floor] separates instrument drift from brand signal across time. A hash-sealed pinned panel of public artifacts, re-read byte-identically under successive model versions, yields a *version floor* — the across-time counterpart of the operator noise floor — while a companion live panel of fresh artifacts restores the ecological quantity. The brand velocity that earlier framings assigned to PRISM-T — the first-order difference $\mathbf{\Delta r} = \mathbf{r}_{t_2} - \mathbf{r}_{t_1}$ and the spectral velocity norm $\lVert \mathbf{\Delta r} \rVert$, connecting to the dynamics framework [@zharnikov-2026z-spectral-dynamics] — is recovered as the live-panel target (live-minus-pinned), now floored so that a "the brand moved" claim must clear the version floor rather than the operator floor alone.

**PRISM-C (Choice)** [@zharnikov-2026bb-prism-c-choice] measures the gap between a model's stated brand perception and its revealed agentic choice. It elicits stated eight-dimension readings, projects each buyer need into the same space, and compares the need-nearest predicted pick with the model's revealed pick in a simulated choice task, judged against the choice elicitation's own cross-family operator floor. Its pre-registered campaign found model families near-unanimous with one another yet diverging from the predicted pick on a large share of trials — many times the floor — establishing that stated-perception measurement does not transfer to agentic choice for free.

**PRISM-O (Organizational)** [@zharnikov-2026bd-prism-o-optimization-depth] carries the scaffold beyond brand perception, measuring the gap between the optimization depth an organization states and the depth its described processes enact, against a cross-family operator floor. It joins the Spectral Brand Theory and Organizational Schema Theory corpora at a shared measurement seam, extending the family's observer-agnostic, floor-disciplined design to organizational self-description.

**PRISM-H (Human)** is a planned human-subject confirmatory instrument that administers the PRISM-B items to human respondent cohorts, supplying the human validation this specification defers (see "Limitations"). It shares the PL0--PL4 scaffold and the 1--5 ordinal format; its campaign is not yet run.
...
The observer-agnostic design aligns with the emerging consensus that AI systems should be evaluated using the same constructs applied to human cognition where applicable [@hagendorff-2023-humanlike-intuitive-behavior; @hashimoto-2025-exploring-personality-structure; @serapiogarcia-2025-psychometric-framework-evaluating]. Martini [-@martini-2026-can-ai-generate] demonstrated that AI can generate psychometric scales with measurable validity properties, providing a methodological parallel to the present work; Stein, Messingschlager, and Gnambs [-@stein-2024-attari12-development-validation] developed the ATTARI-12 scale for measuring attitudes toward AI, illustrating that new psychometric instruments are needed for constructs that did not exist under the traditional paradigm. The argument is not that AI and human perception are identical but that commensurable measurement enables meaningful comparison of how they differ.
...
First, **no human validation data exist for PRISM-B as specified here**. The 1--5 ordinal format is the empirically observed minimum-distortion operating point for AI observers [@zharnikov-2026-empirical-rate-distortion-curve-ai], but its psychometric properties (internal consistency, test-retest reliability, convergent and discriminant validity) for human respondents are unknown. Establishing these properties requires administering PRISM-B to human samples alongside established brand perception instruments [@aaker-1997-dimensions-brand-personality; @brakus-2009-brand-experience-what; @keller-1993-conceptualizing-measuring-managing] and demonstrating that PRISM-B captures distinct variance not reducible to brand personality or brand experience. Human validation is especially critical given evidence that consumers penalize AI-generated content for perceived inauthenticity [@feuerriegel-2024-generative-ai], raising the question of whether AI-derived instrument properties transfer to human respondents who may engage with brand stimuli differently. This is the highest-priority future study; the planned PRISM-H instrument (see "The PRISM Instrument Family") is its vehicle.

Second, **the items are theoretically derived, not empirically generated**. The Churchill [-@churchill-1979-paradigm-developing-better] paradigm for developing marketing constructs, codified in the standard scale-development literature [@devellis-2017-scale-development-theory], begins with a large item pool, administers it to a development sample, and retains items based on factor loadings, inter-item correlations, and fit indices. PRISM-B follows the alternative Rossiter [-@rossiter-2002-coarse-procedure-scale] C-OAR-SE procedure, which argues that for constructs with concrete, observable referents, single-item measures are not only legitimate but preferable to multi-item scales that introduce redundancy. The dimensional taxonomy [@zharnikov-2026-why-eight-completeness-necessity-sbt] provides the structure, and each dimension receives a single item. Single-item-per-dimension instruments trade internal consistency estimation for parsimony and dimensional completeness. The trade-off is justified when the dimensions are conceptually independent and empirically separable [@bergkvist-2007-predictive-validity-multipleitem], but the assumption of independence requires empirical verification.
