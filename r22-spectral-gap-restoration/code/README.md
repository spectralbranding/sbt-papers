...
Produces Figure 1 (Bifurcation diagram of spectral gap vs mu/lambda ratio).
...
Seed: 42. Outputs: `../figures/figure1_bifurcation.png` (300 dpi).
...
### switching_matrix_gap.py

Verifies the Bridging Illustration (Numerical Illustration section): closed-form spectral gap S/(S+1) of the zero-order Dirichlet-multinomial switching matrix, stationarity of the share vector, double-jeopardy diagonal, and share-perturbation half-lives.

**Run command:**
```
uv run --with numpy python3 switching_matrix_gap.py
```

Seed: 2026 (random-share robustness replicate only; the main results are deterministic closed forms). Outputs: stdout table.
