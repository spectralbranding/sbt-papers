...
def run_simulation() -> dict:
    ...
    for cohort in range(N_COHORTS):
        ...
        scores_high = np.stack(
            [
                simulate_perception_score(
                    MU_HIGH, LAMBDA, N_MONTHS, NOISE_STD, rng, x0=rng.uniform(3.0, 7.0)
                )
                for _ in range(8)
            ],
            axis=1,
        ...
        scores_low = np.stack(
            [
                simulate_perception_score(
                    MU_LOW, LAMBDA, N_MONTHS, NOISE_STD, rng, x0=rng.uniform(3.0, 7.0)
                )
                for _ in range(8)
            ],
            axis=1,
        ...
        for dim in range(N_DIMS):
            ...
            # HIGH regime
            sg_h = simulate_spectral_gap(
                MU_HIGH, LAMBDA, N_MONTHS, NOISE_STD, rng, gap0
            )
    ...
    results = {
        ...
        "high_regime": {
            ...
            "adf_pvals_all_cells": [
                round(p, 4) if p is not None else None for p in adf_pvals_high
            ],
            "irf_halflives_all_cells": [
                round(h, 2) if h is not None else None for h in halflives_high
            ],
        },
        "low_regime": {
            ...
            "mean_gap_collapse_month": (
                round(mean_collapse_month, 1) if mean_collapse_month else None
            ),
            "adf_pvals_all_cells": [
                round(p, 4) if p is not None else None for p in adf_pvals_low
            ],
            "irf_halflives_all_cells": [
                round(h, 2) if h is not None else None for h in halflives_low
            ],
        },
        "regime_comparison": {
            ...
            "cosine_drift_ratio_low_over_high": round(
                mean_cd_l / (mean_cd_h + 1e-9), 2
            ),
            "mean_terminal_spectral_gap_high": round(mean_gf_h, 4),
            "mean_terminal_spectral_gap_low": round(mean_gf_l, 4),
            "terminal_gap_ratio_high_over_low": round(
                mean_gf_h / (mean_gf_l + 1e-9), 2
            ),
...
if __name__ == "__main__":
    ...
    out_path = "[internal path removed]"
    ...
    print("\n── Regime comparison ─────────────────────────────────────────────────")
    print(
        f"  Prop. stationary ADF (high-mu):     {rc['prop_stationary_high']:.3f}  "
        f"({hr['n_stationary_adf']}/{hr['n_cells']} cells)"
    )
    print(
        f"  Prop. stationary ADF (low-mu):      {rc['prop_stationary_low']:.3f}  "
        f"({lr['n_stationary_adf']}/{lr['n_cells']} cells)"
    )
    print(f"  Difference:                         {rc['difference']:+.3f}")
    print(
        f"  z = {rc['z_statistic']:.3f},  p (one-sided) = {rc['p_value_one_sided']:.4f}"
    )
    print(
        f"  Mean terminal spectral gap HIGH:    {rc['mean_terminal_spectral_gap_high']:.4f}"
    )
    print(
        f"  Mean terminal spectral gap LOW:     {rc['mean_terminal_spectral_gap_low']:.4f}"
    )
    print(
        f"  Terminal gap ratio (high/low):      {rc['terminal_gap_ratio_high_over_low']:.1f}x"
    )
    print(f"  Mean cosine drift HIGH:             {rc['mean_cosine_drift_high']:.4f}")
    print(f"  Mean cosine drift LOW:              {rc['mean_cosine_drift_low']:.4f}")
    print(
        f"  Mean IRF half-life HIGH:            {hr['mean_irf_halflife_months']} months"
    )
    print(
        f"  Mean IRF half-life LOW:             {lr['mean_irf_halflife_months']} months"
    )
    if lr.get("mean_gap_collapse_month") is not None:
        print(
            f"  Mean spectral gap collapse month:   {lr['mean_gap_collapse_month']:.1f}  "
