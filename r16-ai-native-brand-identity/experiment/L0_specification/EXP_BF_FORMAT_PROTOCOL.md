...
| 2026-04-16 | v2 rerun: fix anchoring confound + sum enforcement | Run 1 found two issues: (a) 41% of records had weight sums outside 95-105 (range 44-177), (b) F5 (score-only vector) produced cos .998 but 89% were near-echoes of input scores — anchoring, not comprehension. F3 (tabular with scores) had 68% near-echoes. Fixes: (1) System + user prompts: "Your weights MUST sum to exactly 100. Verify before responding." (2) Raw scores removed from F3, F4, F5 — only F1 (full JSON) retains numeric scores. F3 now shows dimension + brief positioning text; F4 shows ranked dimensions + rationale (no scores); F5 uses qualitative levels (Very High/High/Moderate/Low/Very Low). This isolates whether structured data with scores (F1) genuinely outperforms verbal-only formats, without anchoring artifacts. Run 1 data preserved as exp_bf_format.jsonl; run 2 at exp_bf_format_v2.jsonl. |
