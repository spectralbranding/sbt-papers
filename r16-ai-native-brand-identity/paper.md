...
Brand identity has always been a function of the observer. When the observer was a mass-market consumer, the logo became the dominant identity technology --- a visual mark enabling instant recognition at scale. As AI agents increasingly mediate purchasing decisions, product recommendations, and brand evaluation, the observer is changing again. This paper argues that cryptographic signatures on behavioral specifications are positioned to replace logos as the primary brand identity mechanism for AI-mediated commerce, following the same historical pattern that produced wax seals (for bureaucratic observers), hallmarks (for asymmetric-information buyers), trademarks (for mass-market consumers), and SSL certificates (for web browsers). We extend Spectral Brand Theory to define the Brand Function --- f(query, context, observer_type, time) -> response --- as the complete behavioral specification of how a brand operates when observed by machines. Drawing on emerging standards (W3C Verifiable Credentials, GLEIF vLEI, .well-known/brand.json proposals) and empirical evidence on LLM brand bias, we propose six formal propositions connecting identity verification technology to observer type, behavioral specification to brand coherence, and cryptographic attestation to trust in autonomous commerce. The paper introduces the concept of behavioral metamerism --- brands with identical statistical profiles but structurally different behavioral signatures --- as the AI-native equivalent of visual brand confusion. We conclude that logos will persist for human observers but become functionally irrelevant for AI agents, and that organizations must develop dual identity infrastructure: visual for humans, cryptographic for machines.

**Keywords**: brand identity, AI agents, cryptographic verification, behavioral specification, metamerism, agentic commerce, generative engine optimization
...
The central practical implication is that brands must develop *dual identity infrastructure*: visual identity for human observers, cryptographic identity for machine observers. This is not an incremental addition to existing brand management but a structural expansion.
...
Brands that prepare for this transition will build dual identity infrastructure --- visual for humans, cryptographic for machines. Those that do not will find their identity increasingly determined by the statistical representations that AI systems construct from available data, rather than by the brand's own specification of what it intends to be.
