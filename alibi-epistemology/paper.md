...
How does knowledge form from observation? The question is ancient — addressed by epistemologists from Peirce's [-@peirce-1931-1958-collected-papers] theory of abductive inference to Goldman's [-@goldman-1986-epistemology-and-cognition] naturalized epistemology. The computational answer is surprisingly recent — and surprisingly underdeveloped.
...
**Stage 2: Cloud Formation.** Atoms from different sources cluster into probabilistic hypothesis clouds — a process structurally analogous to gestalt grouping [@koffka-1935-principles-gestalt-psychology], where discrete perceptual elements are assembled into coherent wholes through proximity, similarity, and closure. A cloud represents "these atoms probably describe the same real-world event." Clouds are scored through weighted multi-dimensional comparison. They are inherently uncertain and continuously revisable as new atoms arrive.
...
**Principle 4: Asymmetric tolerances** acknowledge that "close enough" is context-dependent — a form of fuzzy matching that resonates with Zadeh's [-@zadeh-1965-fuzzy-sets-information] formalization of graded set membership. A receipt and a bank statement for the same purchase might have different dates (the bank processes the charge days later). But an invoice and its payment might be separated by months. The tolerance is not a property of the dimension — it is a property of the source-pair interaction.
...
**Collapse**: The cloud-matching threshold is .50 — a bundle scoring below this is assigned to a new cloud rather than merged with an existing one. The cloud score (.830) comfortably exceeds the threshold. The cloud collapses into a fact: "Confirmed purchase at Starbucks, March 15, $5.20 — receipt $4.95 plus tip."
...
Each of the seven principles has a counterpart in the brand domain, set out below. What follows is the mapping itself, not a test of it. Both the financial architecture and the brand framework it maps onto were designed by the same author, and the brand-domain constructs were derived from the seven principles rather than encountered independently of them — so exhibiting a counterpart for each principle confirms that the mapping was carried out, and nothing beyond that. The section demonstrates that the vocabulary transfers coherently; the limit this places on the transferability claim is stated in the Limitations section, and the independent-replication designs that would test it are named in the Generalizability section.
...
**Condition 4: Stability across prompt variations.** Genuine structural features should be recoverable through multiple prompt formulations. Zharnikov [-@zharnikov-2026-spectral-brand-theory-computational-framework] reports a sensitivity analysis in which five prompt reformulations of the same brand profile analysis produced stable structural classifications (dimension identification, coherence type) while producing variable numerical scores. This asymmetry — structural stability with numerical variability — is the expected signature of a real architectural feature: the underlying signal exists, but its precise quantification is underdetermined by the available evidence.
...
## Discussion: Positioning Against Existing Frameworks

Four requirements define a framework adequate for the multi-domain knowledge-formation problem addressed here: (1) path-independence — conclusions must not depend on the order in which evidence arrives; (2) observer heterogeneity — the framework must natively model multiple observers with different scoring weights; (3) epistemic separation — observations, hypotheses, and knowledge must be structurally distinct objects; and (4) linguistic implementability — the framework must be expressible as natural language instructions for an LLM without requiring formal probability distributions or rule bases. These four requirements are this architecture's own design commitments stated as criteria: each names a property the atom-cloud-fact pipeline was built to have, so the pipeline satisfies all four by construction and its passing score carries no independent weight. What follows is therefore a positioning argument — it locates each rival relative to what this architecture commits to — and not an evaluation of the rivals, which are mature frameworks answering questions this axis does not ask. The part of the argument that does carry content is the character of each shortfall: in every case the requirement is incompatible with a core commitment of the rival, so it could not be met by refinement within that framework, and a reader who rejects the axis is rejecting the architecture's premises rather than disputing a measurement.
...
The path-independence argument also benefits from Pearl's [-@pearl-2009-causality-models-reasoning] causal modeling framework, which identifies the conditions under which evidence-ordering effects propagate through probabilistic networks — conditions that the re-collapse mechanism is specifically designed to break by dissolving the accumulated posterior on each rebuild. Levi's [-@levi-1991-fixation-belief-its] theory of epistemic entrenchment similarly addresses when prior commitments should be abandoned entirely rather than minimally revised; the re-collapse principle can be read as operationalizing Levi's "full contraction" rather than his "partial contraction" — a wholesale rollback to the atom level, not a selective withdrawal of beliefs.
...
### Positioning Against the Four Requirements

The four requirements and five architectures are summarized in Table 5.

**Table 5.** Positioning of Five Architectures Against the Four Requirements.
...
*Notes*: PASS = requirement met by core architecture. PARTIAL = partially addressed with qualifications. FAIL = structural incompatibility. The requirements are this paper's own design commitments rather than a neutral scoring frame, and the final row is the architecture that defines them, so its four PASS entries hold by construction. The table positions the rivals against those commitments; it is not an evaluation of the rivals on their own terms and no ranking is intended. AGM = Alchourrón, Gärdenfors & Makinson [-@alchourrn-1985-logic-theory-change]. MYCIN = Buchanan & Shortliffe [-@buchanan-1984-rulebased-expert-systems].
...
The framework also positions itself against existing alternatives, on requirements that are its own design commitments rather than a neutral axis: Bayesian updating and AGM revision fall short on path-independence through architectural commitments to incremental updating and minimal change respectively; classical expert systems fall short on observer heterogeneity through their global rule architecture; purely probabilistic models fall short on epistemic separation by conflating observation and inference; and all three fall short on linguistic implementability to varying degrees. In each case the shortfall follows from a core commitment of the rival, which is what makes the positioning informative even though the axis is not neutral.
