...
EXPECTED_L4_RESULTS = [
    ...
    "r19_rd_curves.csv",
    "r19_summary.md",
    "r19_pre_registration_audit.md",
    "r19_h4_supplementary.md",
    "r19_jshape_supplementary.md",
    "r19_per_brand_rd.csv",
    "r19_per_brand_supplementary.md",
]

EXPECTED_ROOT_RESULTS: list[str] = []  # R19 has no root-level results JSON files
...
def files_to_checksum() -> list[Path]:
    ...
    extras = [
        ...
        EXPERIMENT_DIR / "L1_configuration" / "brands.yaml",
        EXPERIMENT_DIR / "L1_configuration" / "rate_conditions.yaml",
    ]
...
def check_schemas(verbose: bool = False) -> bool:
    """Validate L3 session records against JSON schema."""
    ...
    # Validate L3 session records (sample first/middle/last + every 100th)
    ...
    for jsonl_path in sorted(l3_dir.glob("*.jsonl")):
        ...
        if n == 0:
            ...
            continue
        # Sample: first, middle, last, plus every 100th
        sample_indices = sorted(set([0, n // 2, n - 1] + list(range(0, n, 100))))
    ...
    if errors_total == 0:
        n_jsonl = len(list(l3_dir.glob("*.jsonl")))
        print(f"Schema validation: PASSED (sampled records from {n_jsonl} JSONL files)")
        return True
    else:
...
def check_completeness() -> bool:
    ...
    if errors == 0:
        ...
        print(
            ...
            f"({n_l3} L3 sessions, {n_l4} L4 results, "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate R19 experiment data.")
