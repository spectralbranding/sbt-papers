...
| **Companion experiment** | `experiments/NEGOTIATION_REAL_CORPORA_SBT_OST_2026-06-13.md` (EXP-2026-06-13-NEG-SBT-OST) — the same instrument on the SBT/OST pair. This record is the next rung up the independence ladder. |
| **Environment** | Python 3.12.13, `uv` 0.11.19; macOS (darwin). Substrate `[internal path removed]` and the live ontology `[internal path removed]*.yaml` present but neither read nor written by the run. |
...
`experiments/negotiation_real_corpora/sbt/sbt_perceptual.yaml` (`paper_key: sbt_real_2026a`),
**reused unchanged** as author A. It owns seven shared SBT perceptual terms, each definition copied
verbatim from the live SBT ontology (`[internal path removed]` + `2026s.yaml`) so that each
...
full live ontology term set (`[internal path removed]*.yaml`, ~190 owned keys). Its defining feature for
...
`experiments/negotiation_independent_aaker/aaker/aaker_brand_equity.yaml` (`paper_key: aaker_real`)
...
**Isolation.** Both module files live only under `experiments/negotiation_real_corpora/` (SBT, reused)
and `experiments/negotiation_independent_aaker/` (Aaker, new). Neither is in `[internal path removed]` nor
named `ONTOLOGY.yaml`, so the live `build_ontology.discover_modules()` (globs exactly
`[internal path removed]*.yaml` and `research/**/ONTOLOGY.yaml`) never discovers them. They are loaded
...
uv run python code/negotiate_modules.py \
    ...
    --author-b experiments/negotiation_independent_aaker/aaker \
    --sssom experiments/negotiation_independent_aaker/sbt_aaker.sssom.tsv
```
...
### 5.1 Tool-emitted SSSOM (mechanical) — `experiments/negotiation_independent_aaker/sbt_aaker.sssom.tsv`
...
### 5.2 Hand-authored curated cross-key SSSOM (the artifact the tool cannot derive) — `experiments/negotiation_independent_aaker/sbt_aaker_crosskey_curated.sssom.tsv`
...
module `experiments/negotiation_real_corpora/sbt/sbt_perceptual.yaml` (reused) and the Aaker input
module `experiments/negotiation_independent_aaker/aaker/aaker_brand_equity.yaml`; (ii) the run command
...
hashing each SBT definition and checkable against `[internal path removed]` + `2026s.yaml`. The
...
- `experiments/negotiation_independent_aaker/aaker/aaker_brand_equity.yaml` — Aaker incumbent author module (new)
- `experiments/negotiation_real_corpora/sbt/sbt_perceptual.yaml` — SBT author module (reused from EXP-2026-06-13)
- `experiments/negotiation_independent_aaker/sbt_aaker.sssom.tsv` — tool-emitted SSSOM (header-only; the mechanical result)
- `experiments/negotiation_independent_aaker/sbt_aaker_crosskey_curated.sssom.tsv` — hand-authored cross-key Rosetta-stone SSSOM
...
(`[internal path removed]` items 37a-37f) and `[internal path removed]`. Until
