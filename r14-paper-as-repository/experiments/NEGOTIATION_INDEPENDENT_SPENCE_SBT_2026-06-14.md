...
| **Companion experiments** | `experiments/NEGOTIATION_REAL_CORPORA_SBT_OST_2026-06-13.md` (EXP-2026-06-13-NEG-SBT-OST — clean federation, CROSS_IMPORT + CROSS_REFINE) and `experiments/NEGOTIATION_INDEPENDENT_AAKER_SBT_2026-06-14.md` (EXP-2026-06-14-NEG-AAKER-SBT — first adversarial class, DANGLING_IMPORT). This record is the third real-corpora run and produces the one class neither prior run did: a genuine same-key CONFLICT. |
...
`experiments/negotiation_independent_spence/sbt/sbt_signal.yaml` (`paper_key: sbt_signal`) **owns four**
...
**verbatim** from `experiments/negotiation_real_corpora/sbt/sbt_perceptual.yaml` (themselves verbatim from
the live SBT ontology `[internal path removed]`), so the `signal` conflict sits in a faithful SBT
...
`experiments/negotiation_independent_spence/spence/spence_signaling.yaml` (`paper_key: spence_real`)
...
**Isolation.** Both module files live only under `experiments/negotiation_independent_spence/`. Neither is
in `[internal path removed]` nor named `ONTOLOGY.yaml`, so the live `build_ontology.discover_modules()`
(globs exactly `[internal path removed]*.yaml` and `research/**/ONTOLOGY.yaml`) never discovers them. They are
...
uv run python code/negotiate_modules.py \
    --author-a experiments/negotiation_independent_spence/sbt \
    --author-b experiments/negotiation_independent_spence/spence \
    --sssom experiments/negotiation_independent_spence/sbt_spence.sssom.tsv
```
...
Written to `experiments/negotiation_independent_spence/sbt_spence.sssom.tsv`. The `DANGLING_IMPORT` carries
...
`experiments/negotiation_independent_spence/sbt/sbt_signal.yaml` and
`experiments/negotiation_independent_spence/spence/spence_signaling.yaml`; (ii) the run command in §3; (iii)
...
(`[internal path removed]`). No seed, dataset download, network call, or API key is required.
...
- `experiments/negotiation_independent_spence/sbt/sbt_signal.yaml` — SBT signal-owner module (new)
- `experiments/negotiation_independent_spence/spence/spence_signaling.yaml` — Spence economic-signaling module (new)
- `experiments/negotiation_independent_spence/sbt_spence.sssom.tsv` — tool-emitted SSSOM (the headline CONFLICT closeMatch row)
...
computation-script publication rule (`[internal path removed]` items 37a-37f) and
`[internal path removed]`. Until that publication the artifacts remain internal-only. This
