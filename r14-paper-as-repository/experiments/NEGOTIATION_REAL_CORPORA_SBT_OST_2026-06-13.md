...
| **Instrument** | `code/negotiate_modules.py` (federated multi-author generalization of the single-author linker `build_ontology.py`), commit `14472c75`. Shared parser `build_ontology.Module` + `def_hash` + `_norm_ref`, commit `2747250e`. |
| **Protocol** | `[internal path removed]`; companion `[internal path removed]`. |
| **Environment** | Python 3.12.13, `uv`; macOS (darwin). Substrate `[internal path removed]` present but not read or written by the run. |
...
- **H2 — Content-addressed identity.** Shared terms are matched by content-addressed `def_hash`,
  ...
  live single-author linker computes over `[internal path removed]` + `2026s.yaml`.
...
`experiments/negotiation_real_corpora/sbt/sbt_perceptual.yaml` (`paper_key: sbt_real_2026a`) owns
seven shared SBT perceptual terms, each definition copied **verbatim** from the live SBT ontology
(`[internal path removed]` + `[internal path removed]`) so that each term's
...
`experiments/negotiation_real_corpora/ost/ost_operational.yaml` (`paper_key: ost_real`) **owns
...
`experiments/negotiation_real_corpora/`. They are **not** in `[internal path removed]` and are **not**
...
uv run python code/negotiate_modules.py \
    ...
    --sssom experiments/negotiation_real_corpora/sbt_ost.sssom.tsv
```
...
Written to `experiments/negotiation_real_corpora/sbt_ost.sssom.tsv`. Rendered as a table, one
...
(`[internal path removed]`) deliberately constructs those harder classes; this
...
Anyone with the repository at tool commit `14472c75` (instrument) / `2747250e` (shared parser) and
Python 3.12.13 can reproduce this record exactly: (i) the two input modules under
`experiments/negotiation_real_corpora/`; (ii) the run command in §3; (iii) byte-identical report and
SSSOM output, since the pipeline is deterministic; (iv) the §2.1 integrity manifest, recomputable
by hashing each SBT definition and checkable against `[internal path removed]` + `2026s.yaml`.
...
- `experiments/negotiation_real_corpora/sbt/sbt_perceptual.yaml` — SBT author module
- `experiments/negotiation_real_corpora/ost/ost_operational.yaml` — OST author module
- `experiments/negotiation_real_corpora/sbt_ost.sssom.tsv` — SSSOM mapping proposal (the
...
(`[internal path removed]` items 37a-37f) and `[internal path removed]`.
