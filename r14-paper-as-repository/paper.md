...
### A Gap Every Other Domain Has Already Solved
...
### The Value Stream: Knowledge Development as the Core Process
...
**Table 2a.** Protocol feature coverage. Columns indicate whether a system supports each of the six protocol features described earlier. Full = structurally integrated; Partial = addressed but not fully integrated; No = not addressed.
...
*Notes*: VC = version control; Fork = fork-based submission; Gate = compliance gate; Rev = reviewer attribution; Prov = provenance chain; Coll = collections as users. Dashes indicate the feature is absent. The six columns are this protocol's own design commitments rather than a neutral feature space: each is a component of the architecture proposed here, derived from the five gaps this paper identifies. Scoring other systems against them therefore measures distance from this design, not merit, and the bottom row is full by construction — a protocol assembled to satisfy six criteria satisfies them. Each of the other systems would score differently under criteria derived from its own aims; Manubot's authoring ergonomics, JOSS's reviewer culture and RO-Crate's packaging generality are all outside the columns entirely. Ratings are the author's reading of each system's public documentation, not the developers' own assessment.
...
**Figure 1.** The four-level Research-as-Repository hierarchy. Each level is a structural transformation of the level above, preserving cryptographic provenance. The dashed line shows publication metadata flowing back to the source repository, closing the loop and supporting the rendering-isomorphism.
...
JATS XML (Journal Article Tag Suite) is the dominant machine-readable format for published articles. The protocol does not replace JATS — it generates JATS as one rendering output from the repository source. The submission gate can include JATS validation as a compliance check. The relationship is: the repository stores the source; JATS is one of several delivery formats the CI pipeline produces.
...
### Fork-Based Submission and the Hybrid Flow
...
### The Submission Gate: Compliance as CI/CD
...
blinding:
  mode: double-blind
  # ... (detailed blinding config as described below)
...
**Implementability.** Every check in the gate is technically straightforward (Table 3):
...
JOURNAL FORK
    ...
    |
    |--- review/reviewer-1
    |    |-- commit: "Methodology section critique" (2026-04-01)
...
### Reviewer Ownership and Portable Review History
...
### Funding, Affiliations, and Provenance Metadata
...
### Toward a Normative Specification
...
**API endpoints.** The federation protocol requires a defined interface — whether REST endpoints, protocol-level messages, or both — through which independent hosts exchange provenance metadata, fork requests, and decision records. DataCite's REST API for DOI minting and metadata retrieval provides a model for the provenance query interface. ORCID's OAuth-based identity verification provides a model for the authentication flow.
...
### The Research Wiki: Structured Knowledge Accumulation
...
- **Independence proof**: whether an idea was developed independently — if another group's preprint never appears in `ingest.jsonl`, the log holds no record of the researcher having encountered it. This one is weaker than the other four and should not be called a proof. `ingest.jsonl` is written by the party whose independence is at issue, and it records only the additions that party chose to route through the tooling. Reading a preprint outside the wiki, or removing an entry before the first push, leaves the log silent in exactly the way genuine independence does. Git's hashing makes the log tamper-evident after the fact; it cannot make the log complete at the time of writing, and no cryptographic property supplies an entry that was never made. What an absent entry supports is narrower: that the party asserting independence committed to a contemporaneous record before any dispute arose, and that the record is consistent with the assertion. That is better than an informal assertion, and it is not a demonstration.

#### Privacy and the Commit-Reveal Mechanism
...
Every feature that the current system implements through policy, the repository protocol implements through structure (Table 4):
...
### Minimal Mandatory Transparency, Maximal Optional Transparency
...
### For Universities and Research Evaluation
...
Table 6 summarizes how the protocol addresses each stakeholder's current pain.
...
The combination — a link-time compatibility check plus a build-failing cross-artifact drift gate, operated continuously over one corpus — is the part of the realized system with the least precedent, and a later section isolates why.

### Spine-First Drafting Is the Generative Discipline
...
This paper is itself drafted under that discipline. Its structured spine was built before this section's prose; every claim in this section traces to a node in it; the bundle that travels with the paper — its prose, its spine, its ontology module, and its rendered glossary — is checked by the same gate described above. The self-reference of the rendering-problem isomorphism is thus not only rhetorical: the manuscript is a render of the substrate, and the render is held consistent with its source by the running system the section describes.

### Relationship to Existing Standards: What Is Emitted, What Is New
...
The protocol's seven claims are conceptual propositions, not empirically validated results. Future pilot implementations should be evaluated against concrete, measurable success criteria. The following metrics define what "working" means for each structural innovation:

**Desk rejection rate reduction.** The compliance gate should eliminate formatting-related desk rejections entirely. Target: 80% reduction in desk rejections attributable to formatting, metadata, or compliance failures at adopting journals. Measurement: compare desk rejection rates (and reasons) for git-linked submissions versus traditional submissions at the same journal over a 12-month period.
...
**Reviewer engagement depth.** The review branch model makes reviewer effort structurally visible. Engagement depth can be measured via commit count, diff size (lines of substantive commentary), and time-on-review (first commit to final recommendation). These metrics do not measure review *quality* — a short, incisive review may be more valuable than a long, unfocused one — but they provide a quantitative baseline that does not currently exist. Target: establish normative ranges for review depth across disciplines as a foundation for reviewer recognition.

**Dual submission detection.** The provenance chain makes concurrent submissions structurally detectable within the protocol. Target: 100% structural detection of concurrent identical submissions to journals that participate in the federation protocol. Measurement: false negative rate (submissions that evade detection) and false positive rate (legitimate parallel submissions, e.g., to a preprint server and a journal, incorrectly flagged).

**AI contribution traceability.** The AI-native layer records AI-assisted edits as attributed commits carrying tool name, model version, and prompt hash. Unlike the metrics above, this target cannot be evaluated as stated, and the obstruction is structural rather than practical. The natural measurement is the percentage of AI-assisted edits that carry complete attribution metadata — but the denominator, the set of AI-assisted edits, is observable only through the attribution metadata that constitutes the numerator. An AI-assisted edit committed without attribution is not a missing entry in a known population; it is absent from the population as the protocol can see it. The ratio is therefore unity by construction whatever the true attribution rate, and it carries no information about coverage. Comparing declared AI involvement against commit-log evidence inherits the same limit: it detects a declaration in excess of the log, never a use in excess of both.

A measurable version of this target needs a denominator established outside the log, and two are available to a pilot. The first is an audit sample: draw commits at random, have their authors reconstruct under a disclosure amnesty how each was produced, and estimate the attribution rate against that reconstruction. The second is instrumentation at the editing tool rather than at the commit, so that the record of a model invocation is produced by a party other than the one deciding whether to declare it. Neither is part of the protocol as specified. Until one of them exists, AI contribution traceability is a design intention rather than a metric, and it is listed here as such.
...
### Stakeholder Incentives and Barriers
...
**Editors.** The compliance gate eliminates formatting desk rejections — a direct time savings. The reviewer branch model provides structured data on reviewer engagement — useful for identifying reliable reviewers. Editors benefit immediately from the gate and incrementally from the review model. The hybrid flow provides a transitional compatibility mode, but the git-native flow gives editors structural advantages — version history, contributor verification, and diff capability — that the traditional flow cannot match.

**Reviewers.** The portable review portfolio creates career credit for an activity that currently generates none. The privacy controls allow reviewers to accumulate reputation without revealing specific reviews. The risk is that visible review depth creates pressure to over-invest in each review; the mitigation is that depth metrics are aggregate, not per-review.
...
### Ethics, Legal, and Equity Considerations
...
**Privacy and the irrevocable record.** The provenance chain is append-only by design — this is what makes it trustworthy. But irrevocability means that submission history, rejection patterns, and review timelines are permanently recorded. A researcher who submits to ten journals before acceptance has a visible record of nine rejections (or at minimum, nine prior forks). The protocol's current design makes fork *existence* always visible while keeping *decisions* optionally visible (Limitation 5). The appropriate default — how much provenance is visible to whom — should be community-configured and may vary by discipline. Clinical research, where transparency serves patient safety, may require fuller disclosure than humanities, where submission patterns carry different professional implications.
