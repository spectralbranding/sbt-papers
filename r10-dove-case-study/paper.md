...
The analysis examines Dove's emission profile at four time points: **2003** (pre-campaign), **2006** (early activation: "Evolution" viral video), **2013** (peak resonance: "Real Beauty Sketches"), and **2023** (normative absorption: body positivity mainstreamed). Figure 1 maps these cross-sections against the brand's revenue milestones and Cultural dimension decay.
...
**Skeptic-Critic: Cloud inversion.** The higher the Ideological signal intensity, the more salient the perceived gap between Dove's messaging and Unilever's portfolio. Every emotional beat that strengthened PA conviction strengthened SC negative conviction — the same signal, operating on the same high-weight dimension, producing opposite trajectories. Table 3 summarizes each cohort's perception cloud formation state across the four time points.
...
What the three comparison cases show is coverage rather than prediction, and the distinction matters enough to state. Between them the four accounts — success from null-dimension activation with coherence (Dove), success from reinforcing existing dimensions with product anchoring (Patagonia), failure from null activation without narrative foundation (Pepsi), and failure from contradicting existing dimensions (Gillette) — assign each outcome to a mechanism after that outcome is already known. Each mechanism was selected for its case, and the set is closed under outcome: an activation that succeeded is classed as coherent, one that failed as either lacking narrative foundation or contradicting an existing dimension. No campaign in this sample could have come out in a way the four accounts jointly fail to absorb, so their joint fit to the sample is not evidence.

The set is not empty of content, but the content lies in what it would have to be revised to accommodate. Two patterns would require a mechanism the framework does not contain. The first is a campaign that activates a previously null dimension with no narrative contradiction and a portfolio consistent with it, and still fails commercially — Dove's configuration with Dove's outcome reversed. The second is its mirror: a campaign that contradicts an established dimension, in a portfolio that contradicts it further, and succeeds. Neither occurs in the four cases assembled here, and both are observable in a multi-campaign panel of the kind P1's falsification condition specifies. That neither occurs in a sample of four selected cases is a fact about the sample.
...
Keller's [-@keller-1993-conceptualizing-measuring-managing] equity-as-stock metaphor implies that equity changes quantitatively but not in type. In SBT, Dove's 2004 campaign created a qualitatively new dimension — a state change, not a quantity change. Kapferer's [-@kapferer-2008-new-strategic-brand, 4th ed.] identity prism describes brand identity but is fundamentally sender-focused: it describes what the brand *intends* to communicate, not how different observers *receive* those communications. Table 6 summarizes which of these phenomena each framework can represent.
...
*Falsification: P1 is falsified if purpose campaigns activating a previously null dimension produce equivalent or lower revenue growth than campaigns intensifying existing dimensions, controlling for campaign spend and category baseline growth. Operationally, the test requires: (a) coding campaigns in a multi-brand sample as "null-activation" (dimension transitions from 0 to positive at $t_0$) versus "intensity-amplification" (dimension score increases on an already-positive baseline) using a pre-registered coding protocol; (b) measuring revenue growth in the 24-month window following campaign launch; and (c) finding no statistically significant advantage (p > .05, Cohen's d < .20) for null-activation campaigns over intensity-amplification campaigns in the same category. Dove's Ideological trajectory (null in 2003 $\to$ 8.0 in 2006) would serve as the positive exemplar; campaigns that intensify an already-active dimension without opening new dimensional territory would serve as the comparison set.*
...
### *Basis Dependence and Velocity Tracking*

An earlier version of this section reported that the dimensional activation patterns replicate across alternative dimensional decompositions and were therefore invariant under basis transformations. That claim is withdrawn. No alternative decomposition is defined, computed, or reported anywhere in this paper, and none was tested; the sentence described a robustness check that had not been run.

What can be said instead is narrower, and it cuts against invariance rather than for it. A null-to-positive transition is a coordinate-wise property: it says that one component of the emission vector was zero and then was not. Nullity of a component is not preserved under a general rotation of the basis, so the Ideological activation finding, the Cultural inverted-U, and the PA/SC mirror divergence are all stated in the SBT-8 basis and are not known to survive redescription in another. Whether they do is an open empirical question with a straightforward test: recode the same source material under an independently motivated alternative decomposition, recompute the trajectory, and check whether a state change appears at the same time point. Until that is done, the case findings should be read as findings about Dove's trajectory in the SBT-8 coordinate system, and their generality beyond it is unestablished.
