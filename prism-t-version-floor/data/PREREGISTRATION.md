...
protocol) of the PRISM-T temporal study; it is *not* the paper. This preregistration is the
data-collection protocol, frozen before any drafting begins. Companion: `INSTRUMENT_SCAFFOLD.md`.
...
- **9.3 Panel.** The pinned panel reuses the frozen 40-brand stratified bank
  (`[internal path removed]`, prism-m/v1.0.0: five coherence types ×
  ...
  byte-identical with SHA-256 manifest under `[internal path removed]`. This exceeds
  the §3 minimum (≥ 30 × ≥ 4).
