...
|----------------|---------|-----------------|-------------|----------------|
| Ecosystem (A+) | Hermes | $\rho$-isotropic | 8 | Mixed |
...
**Remark.** Theorem 1 isolates the distributional effect by comparing drift fields with equal total magnitude $\sum_i \alpha_i$. In practice, coherence types also differ in total emission intensity (e.g., $\sum_i s_i = 62.5$ for Hermes vs. $44.0$ for Tesla), which creates a confound between drift distribution and drift level. The stability ordering of Theorem 1 holds after controlling for total drift; the empirically observed ordering reflects both effects.
