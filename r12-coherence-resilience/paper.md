...
The remainder of the paper proceeds as follows. The analysis first characterizes the drift structures produced by each coherence type, then analyzes the stability properties of each drift structure. It next models crises as perturbations and derives recovery probabilities, assembles the full predicted ordering into a coherence-resilience theorem, applies the framework to four documented brand crises, and states falsifiable predictions for empirical testing.

## Drift Structure by Coherence Type

### The SDE on $S^7_+$

**Prior stochastic models in marketing.** Marketing Science has a long tradition of modeling brand and customer dynamics with stochastic processes. Naik and Raman [-@naik-2003-understanding-impact-synergy] formalize synergy effects in multimedia advertising using a state-space model driven by a stochastic differential equation, establishing the ODE/SDE toolkit as standard for multi-channel brand-signal dynamics. Schweidel, Fader, and Bradlow [-@schweidel-2008-understanding-service-retention] model service retention as a latent absorbing process, where customers defect once an unobserved threshold is crossed — the absorbing-boundary logic that the present paper extends to perceptual dimensions. Pauwels, Silva-Risso, Srinivasan, and Hanssens [-@pauwels-2004-new-products-sales] use vector autoregressive impulse-response analysis to trace how product-launch and promotion shocks propagate through sales and firm value over time, providing an empirical anchor for the perturbation-and-recovery structure formalized here. These models treat brand strength as a unidimensional scalar or a low-dimensional latent state. The present paper extends this tradition to an eight-dimensional perceptual manifold with dimension-specific absorbing boundaries, demonstrating that the geometry of drift — not merely its magnitude — governs crisis resilience.

We recall the framework of Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-diffusion]. An observer's perception profile $x(t) \in S^7_+$ evolves according to the Stratonovich SDE:
...
where $\gamma > 0$ is a coupling constant and $h(x_i)$ is a modulating function satisfying $h(0) = 0$ (no drift at the absorbing boundary) and $h'(x_i) > 0$ (drift increases with current perception level). Throughout this paper, we adopt the Wright-Fisher-type linear specification $h(x_i) = x_i$, so that $\alpha_i(s, x) = \gamma s_i x_i$. This choice satisfies the Feller boundary condition [@ethier-1986-markov-processes-characterization, Chapter 8], produces a diffusion that is well-posed with $0$ as a regular absorbing boundary, and is the specification under which the convexity premise required by the proof of Theorem 1 is stated (Appendix A, where it is asserted rather than verified). The decay drift is:
...
**Definition 1** ($(\alpha_{\min}, k)$-robust drift). A signal-driven drift field $\alpha$ is **$(\alpha_{\min}, k)$-robust** if at least $k$ of the eight dimensions satisfy $\alpha_i \geq \alpha_{\min} > 0$. Equivalently, the active set $A = \{i : \alpha_i \geq \alpha_{\min}\}$ has cardinality $|A| \geq k$. The robust-drift criterion fixes an absolute floor $\alpha_{\min}$ on protected dimensions rather than a relative ratio. Robustness is highest when $k = 8$: every dimension receives drift above the floor, leaving no vulnerability corridor. We write $k = k(\alpha; \alpha_{\min})$ for the number of dimensions clearing the floor at threshold $\alpha_{\min}$.

The floor is derived from the model's own two drift components rather than assigned. On dimension $i$ the signal-driven term contributes a rate $\partial \alpha_i / \partial x_i = \gamma s_i$ to the deterministic flow, and the decay term contributes a restoring rate $\partial |\beta_i| / \partial x_i = \delta$. The two are equal at the critical emission

$$s_{\text{crit}} = \frac{\delta}{\gamma}, \qquad \alpha_{\min} = \gamma \cdot s_{\text{crit}} \cdot h(x_i^*) = \delta \cdot h(x_i^*)$$

Below $s_{\text{crit}}$ the deterministic flow on dimension $i$ has a finite decay-anchored equilibrium at $x_i = \delta x_i^* / (\delta - \gamma s_i)$, so the dimension's resting position is set by the brand-independent prior; at or above $s_{\text{crit}}$ no such equilibrium exists below the feasibility boundary and the dimension is carried by the brand's own emission. At the working parameters of the companion simulation ($\gamma = .020$, $\delta = .050$) this gives $s_{\text{crit}} = 2.5$. A drift field with $k = 8$ at this floor is **isotropic in the operative sense**: all dimensions are protected.
...
### Mapping Coherence Types to Drift Structures

We now map each of SBT's five coherence types to a drift structure using the canonical emission profiles [@zharnikov-2026-brand-space-geometry-formal-metric].

**Ecosystem coherence (A+).** The canonical example is Hermès, with emission profile $s = (9.5, 9.0, 7.0, 9.0, 8.5, 3.0, 9.0, 9.5)$. The minimum emission is $s_6 = 3.0$ (Economic), with all other dimensions above 7.0. The signal-driven drift field is isotropic in the operative sense: all eight dimensions clear the derived floor $s_{\text{crit}} = 2.5$, the weakest being $s_6 = 3.0$. The drift field is therefore $(\alpha_{\min}, 8)$-robust: no vulnerability corridor exists. The maximum-to-minimum ratio is $9.5/3.0 \approx 3.17$, which would correspond to a comparatively large value under a ratio-based isotropy measure; the operative criterion treated here is the floor-crossing one, since absorption risk is governed by the weakest dimension's absolute drift, not by the spread of the drift vector.

Clearing the floor on all eight dimensions is not, however, a property that distinguishes this type from the next three. IKEA and Patagonia clear the same floor on all eight dimensions (weakest emission 5.0 in both profiles), and Erewhon clears it on all eight as well, its weakest emission of 2.5 sitting exactly at $s_{\text{crit}}$. Only Tesla falls below, on Temporal ($s_8 = 2.0$), giving $k = 7$. The floor-crossing count therefore separates the incoherent type from the other four and does not order those four among themselves. The finer distinctions drawn in the remainder of this section rest on the graded anisotropy of Definition 2, applied descriptively to the shape of each profile, and this should be read as a classification of the canonical profiles rather than as a second consequence of the floor.
...
### Summary of Drift Typology

Table 1 collects the drift typology, mapping each coherence type to its drift structure, the number of active dimensions, and the intrinsic volatility class of those active dimensions.

**Table 1.** Drift Typology by Coherence Type.
...
*Notes*: The drift-structure labels and the active-dimension counts are a descriptive reading of relative drift concentration in each canonical profile, taken at an informal strong-drift level of $s_i$ around 7. They are not floor-crossing counts $k(\alpha; \alpha_{\min})$ at the derived floor $s_{\text{crit}} = 2.5$, which are 8 for Hermès, IKEA, Patagonia and Erewhon and 7 for Tesla. Dimension volatility is the stipulated ordering of Definition 3.
...
### Absorption Risk and Drift Geometry
...
For coordinate $i$, the distance to the absorbing boundary is $x_i(t)$ itself. The process is well-defined as a Markov diffusion under standard regularity conditions [@ethier-1986-markov-processes-characterization, Chapter 8]. Under the SDE, the expected instantaneous change in $x_i$ is:
...
### Stability Ordering by Drift Isotropy
...
If $p(T; \alpha)$ is convex in $\alpha$ (the marginal benefit of additional drift diminishes — pushing a dimension that is already far from the boundary is less valuable than pushing one that is close), Jensen's inequality implies that concentrating drift on fewer dimensions increases the total absorption probability. **Convexity is asserted here, not established.** What the scale-function argument for the Wright-Fisher-type diffusion and the comparison theorem for one-dimensional diffusions [@ikeda-1989-stochastic-differential-equations, Theorem VI.1.1] deliver is *monotonicity* of the hitting probability in the drift parameter, which is a weaker property and does not license the Jensen step. Appendix A records a heuristic for the second-order property and defers its verification; Theorem 1 and everything that rests on it are conditional on that verification. The stochastic drift case follows similarly: temporal variance in $\alpha_i$ increases absorption risk by the same convexity argument applied across time rather than across dimensions. $\square$
...
### The Role of Dimension Volatility
...
**Definition 3** (Dimension volatility ordering). We define the **intrinsic volatility** $\nu_i$ of dimension $i$ as the diffusion coefficient scale in the absence of signal-driven drift. The ordering below is *stipulated*. It is not measured here and it is not established elsewhere in this research program:

$$\nu_{\text{Tem}} < \nu_{\text{Sem}} < \nu_{\text{Nar}} < \nu_{\text{Ide}} < \nu_{\text{Cul}} < \nu_{\text{Eco}} < \nu_{\text{Soc}} < \nu_{\text{Exp}}$$

This ordering reflects a structural property of the underlying perceptual processes. Temporal perception (heritage, longevity) changes slowly — a brand's 200-year history does not fluctuate day to day. Semiotic perception (visual identity) is anchored by physical brand assets. At the other extreme, experiential perception (product interaction quality) fluctuates with each encounter, and social perception (community, status) shifts with cultural trends. Two endpoints of the ordering have external support: brand heritage is the most stable component of brand equity [@urde-2007-corporate-brands-with] and experiential associations are the most volatile [@brakus-2009-brand-experience-what]. The six intermediate ranks do not, and neither does the low-volatility placement of the Ideological and Narrative dimensions, which is what the identity-versus-experiential-asymmetry comparison below actually turns on. The nearest quantity available within this research program is the cross-model variance in the weight observers allocate to each dimension reported by Zharnikov [-@zharnikov-2026-why-eight-completeness-necessity-sbt, Table 4], and it is a different construct: it measures disagreement about how much a dimension matters, not the rate at which perception on that dimension fluctuates over time. Its ranking also does not reproduce the ordering above, placing Social and Experiential among the least variable dimensions rather than the most. No $\nu_i$ estimate is therefore imported from it, and the ordering is carried as an assumption for the remainder of the paper. Estimating $\nu_i$ would require dimension-level perception tracking over time, which is not available here.
...
*Proof sketch.* High intrinsic volatility on the active dimensions does not improve stability because the strong drift is partially offset by large random fluctuations. Low intrinsic volatility on the active dimensions means that the strong drift operates in a low-noise environment, producing a more deterministic trajectory away from the boundary. Meanwhile, the passive dimensions (those without strong drift) are vulnerable to absorption regardless of their volatility — but high-volatility passive dimensions are more vulnerable than low-volatility ones. The optimal configuration is therefore strong drift on low-volatility dimensions and weak drift on high-volatility dimensions — which is the ecosystem case (all dimensions protected). The worst configuration among equal-$k$ brands is strong drift on high-volatility dimensions. $\square$

The corollary as stated is a comparative claim in $\nu$, and that much is derived. Which of the eight SBT dimensions are the low- and high-volatility ones is not derived, and enters only through the stipulated ordering of Definition 3. Every application of the corollary to a named pair of coherence types below — and in particular the placement of experiential asymmetry below identity coherence — is therefore conditional on that stipulation. If the ordering were reversed on the Ideological, Narrative, Social and Experiential dimensions, the corollary would place the two types the other way round with no change to its argument.

### Full Stability Ordering
...
3. **Identity < Experiential asymmetry**: Both are approximately 3-anisotropic ($k \approx 3$), so Theorem 1 alone does not distinguish them. The distinction comes from Corollary 1: Patagonia's active dimensions are Ideological and Narrative, placed low in the stipulated volatility ordering, while Erewhon's are Experiential and Social, placed high. Same drift coverage, different stability under that ordering. The step is conditional on Definition 3 and is not established by the model.
...
This ordering is exactly SBT's coherence grading: A+ < A- < B+ < B- < C- (in absorption probability), or equivalently A+ > A- > B+ > B- > C- (in stability). $\square$

The recovery of the grading should be read for what it is. Steps 1, 2 and 4 apply Theorem 1 to the descriptive anisotropy labels of Table 1, and step 3 applies Corollary 1 to the stipulated volatility ordering of Definition 3. Both inputs are read off the same five canonical emission profiles that define the coherence types in the first place, and neither is an independent measurement: the floor-crossing criterion, once derived from $\delta / \gamma$ rather than assigned, separates only the incoherent type. The ordering therefore restates the type assignment in dynamical vocabulary rather than testing it. What the derivation contributes is the mechanism — absorption governed by the weakest dimension, under a convexity premise that Appendix A asserts rather than establishes — and a set of conditions under which the ordering would fail. Establishing the ordering as a result requires the measured emission profiles and the tracking data called for in the predictions below.

## Crisis as Perturbation

### Modeling a Brand Crisis
...
### Recovery Probability by Coherence Type
...
### Recovery Trajectory Schematics

The active-versus-passive distinction in Theorem 2(c) admits a geometric reading. A single dimension's perception trajectory after a unit shock follows a qualitatively different path depending on whether (i) the dimension receives substantial signal-driven drift ($\alpha_i$ large) or is essentially passive ($\alpha_i \approx 0$), and (ii) the brand's overall coherence type provides corroborating drift on adjacent dimensions or not. The four limiting cases are illustrated in Figure 1.
...
**Figure 1.** Schematic recovery trajectories for four coherence types. Active dimensions (left column) regain function after shock by virtue of substantial signal-driven drift; passive dimensions (right column) drift only weakly toward the neutral prior $x^*$ and may be absorbed at the boundary $x_i = 0$. Asymmetric coherence types (bottom row) compound recovery delays because adjacent dimensions cannot supply corroborating drift through cross-dimensional coupling. Schematic only — see the Companion Computation Script for parameter values producing the underlying ordering.

The schematic captures the qualitative content of Theorem 2(c): on an active dimension the deterministic component dominates the diffusive component near the post-shock state, so trajectories return to a neighbourhood of equilibrium; on a passive dimension the diffusive component dominates and a non-trivial fraction of trajectories absorb before reaching the neutral prior. The Monte Carlo run distributed in `code/coherence_resilience_mc.py` reproduces this pattern for the five canonical brands across four shock severities; it simulates the model rather than testing it against data.
...
- **Incoherent**: Approximately spherically symmetric vulnerability, but at a low level of resilience everywhere. Tesla is not especially vulnerable in any particular direction — it is vulnerable in all directions. When incoherence is associated with brand hate [@hegner-2017-determinants-outcomes-brand], the boundary condition is reinforced by active negative signaling, further elevating the absorption risk on multiple dimensions simultaneously.

### Connection to Recovery Time

The recovery *probability* is complemented by the recovery *time*. Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-diffusion, Proposition 6] established that for the semi-permeable boundary model, the recovery time from a near-boundary state at distance $\delta$ from the absorbing boundary scales as:
...
## The Coherence-Resilience Theorem

The logical chain from a brand's emission profile to its crisis-resilience ordering, developed across the preceding sections, is summarized in Figure 2.
...
*Notes*: The chain proceeds from the emission profile $s$ to the signal-driven drift field $\alpha = \gamma \cdot s \cdot x$ (using the Wright-Fisher specification $h(x_i) = x_i$), to a classification of drift geometry by floor-crossing count $k$ and active-dimension volatility, and finally to the absorption-probability ordering of Theorem 1, conditional on the convexity premise asserted in Appendix A, refined by Corollary 1 under the stipulated volatility ordering of Definition 3. The five canonical brands (Hermès, IKEA, Patagonia, Erewhon, Tesla) instantiate one type each.

### Assembling the Full Prediction

This section combines the stability analysis and the crisis recovery analysis into a single theorem that formalizes SBT's coherence-resilience prediction.

**Theorem 3** (Coherence-Resilience Theorem). *Under the SDE model of Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-diffusion] with drift parametrized by coherence type as established earlier:*
...
The type-over-score principle also explains why traditional brand equity measures — which typically aggregate across dimensions into a single score — fail to predict crisis outcomes. A high aggregate score is compatible with both ecosystem coherence (uniform high) and signal coherence (concentrated high with gaps). The aggregate score cannot distinguish these cases; coherence type can. This is the brand-equity analog of the spectral metamerism construct of Zharnikov [-@zharnikov-2026-spectral-metamerism-brand-perception-projection]: two brands that are perceptually indistinguishable on aggregate measures can produce divergent crisis trajectories, just as two spectral distributions that are visually metameric under one illuminant diverge under another.
...
*Inline numerical values* — the per-brand emission sums $\sum_i s_i$, within-brand variance $\text{Var}(s_i)$, and the floor-crossing count $k(\alpha; \alpha_{\min})$ at the derived threshold $s_{\text{crit}} = \delta / \gamma = 2.5$ — are reproduced by `coherence_resilience_computations.py` (no arguments; reads the five canonical profiles; prints per-brand total emission, variance, mean, and floor-crossing count). Run command: `uv run python code/coherence_resilience_computations.py`.

*Monte Carlo verification* of the qualitative orderings in Theorem 2 and the recovery-trajectory schematics is provided by `coherence_resilience_mc.py` (fixed seed `1729`; 2,000 paths per cell; Euler-Maruyama discretization with absorbing boundary at $x_i = 0$ and reflecting boundary at $x_i = 1$; horizon $T = 60$ months; working parameters $\gamma = .020$, $\delta = .050$, $\sigma_0 = .080$, $dt = .25$ months). Run command: `uv run python code/coherence_resilience_mc.py`. The output `data/results.json` records per-brand per-dimension absorption probability, recovery probability, and median recovery time across four shock severities (.05, .10, .15, .20 absolute drop). At the working parameters the active-dimension recovery is essentially certain ($P_{\text{rec}} = 1$) for all brands except Tesla on its weakest dimension (Temporal, $s = 2$), where $P_{\text{rec}} = 0$ within the 60-month horizon — a numerical instance of the type-over-score principle: Tesla's incoherent emission profile leaves its weakest dimension without sufficient drift for active recovery, regardless of total emission magnitude. Appendix A does not supply a proof of the absorption-probability ordering of Theorem 1: it records a heuristic for the convexity premise and defers the verification to a companion note. The present script covers the recovery-probability and recovery-time orderings of Theorem 2 only.

## Application to Known Crises
...
Observed outcome: BP's recovery has been incomplete more than a decade later. The brand permanently abandoned the "Beyond Petroleum" positioning, acknowledging the gap between narrative and ideology. The company paid over \$65 billion in cleanup costs, fines, and settlements. Brand perception studies [@shamma-2009-customer-noncustomer-perspectives] showed that stakeholder and non-customer audiences exhibited markedly slower reputational recovery for BP than for comparably sized companies experiencing less severe crises. The slow, partial recovery is consistent with the prediction: experiential-asymmetry brands lack the drift reserves to recover from crises on their unprotected dimensions.
...
**Statistical test**: Hierarchical regression with recovery speed as the dependent variable. Model 1 includes only coherence score (sum of emissions). Model 2 adds coherence type (categorical: A+, A-, B+, B-, C-). The prediction is that the nested-model $F$-test rejects, and that the improvement survives a penalty for the four added parameters: adjusted $R^2$ increases, and cross-validated $R^2$ under $k$-fold partitioning by brand increases. An unadjusted $\Delta R^2 > 0$ is not part of the prediction, because it holds for any added regressor including a random one and therefore carries no falsifying content.

**Null hypothesis**: Coherence score alone is sufficient; type adds no explanatory power beyond the parameters it costs. If the $F$-test does not reject, or if the adjusted and cross-validated $R^2$ do not increase, Theorem 3 is falsified.
...
### Contribution to Crisis Management Theory

The crisis management literature has developed sophisticated frameworks for classifying crisis response strategies [@coombs-2007-protecting-organization-reputations], measuring the moderating role of prior reputation [@dutta-2011-effectiveness-corporate-responses], and analyzing the spillover effects of crises on competitors and category perceptions [@cleeren-2017-marketing-research-productharm]. What has been missing is a structural account of *why* some brands are more resilient than others, independent of their crisis response. This paper provides such an account.
...
This finding challenges the implicit assumption in much brand equity research that brand equity is a scalar quantity. Measures like brand value (Interbrand), brand power (Millward Brown), or brand stature (Young & Rubicam) aggregate multi-dimensional assessments into a single number. The Coherence-Resilience Theorem shows that this aggregation destroys precisely the information that predicts crisis outcomes. This parallels Molenaar's [-@molenaar-2004-manifesto-psychology-as] argument in psychology that ensemble-average analyses systematically misrepresent individual trajectories — the same failure mode afflicts aggregate brand equity scores when applied to individual brand dynamics during crises.

### Connection to the SBT Research Program

This paper fills gap BS-6 in the Spectral Brand Theory research program. The connection between coherence type and crisis resilience was posited and explored qualitatively in Zharnikov [-@zharnikov-2026-spectral-brand-theory-computational-framework], formalized dynamically in Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-diffusion], and extended to non-ergodic perception in Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-why]. This paper completes the chain by providing the formal derivation.

The paper also connects to Zharnikov [-@zharnikov-2026-spectral-resource-allocation-demand-driven], which analyzed spectral resource allocation — the question of how brands should distribute investment across dimensions. The present paper provides a crisis-specific rationale for the broad-spectrum allocation strategy recommended there: brands that allocate resources to protect all dimensions (ecosystem coherence) are structurally more resilient than brands that concentrate resources on a few dimensions (signal or identity coherence).

### Limitations

Five limitations merit acknowledgment.

First, the coherence-type classifications used in the case studies are author-assigned qualitative assessments, not empirical measurements. Until validated measurement instruments for the eight SBT dimensions are available, the framework's predictions cannot be rigorously tested. The paper specifies what data is needed and what tests apply, but the tests themselves remain to be conducted.
...
The proof sketch for Theorem 1 invokes the convexity of the hitting probability $p(T; \alpha)$ in the drift parameter $\alpha$ for a one-dimensional diffusion with absorbing boundary at zero. **The property is asserted, not verified.** What follows is a heuristic for it, together with the one adjacent property that *is* established — monotonicity. A rigorous derivation of convexity, together with numerical verification by Monte Carlo simulation of the full eight-dimensional SDE, is left to a forthcoming technical companion note. Until that note exists, Theorem 1 should be read as conditional on the assertion made here.
...
and the hitting probability admits a representation in terms of $S$ and the speed measure [@karlin-1981-second-course-stochastic, Ch. 15]. Under the Wright-Fisher specification, the comparison theorem of Ikeda and Watanabe [-@ikeda-1989-stochastic-differential-equations, Theorem VI.1.1] establishes stochastic monotonicity of the diffusion in $\alpha$: for $\alpha_1 < \alpha_2$, the process started at $y_0$ with drift coefficient $\alpha_1$ is stochastically dominated by the process with $\alpha_2$, hence $p(T; \alpha_1) \geq p(T; \alpha_2)$ — monotonicity of the hitting probability in $\alpha$.
...
The present paper therefore records the convexity claim as **asserted and pending verification**. This appendix does not verify it: it supplies a heuristic for the second-order property, states the computation that would settle it, and defers that computation to the companion note. Monotonicity in $\alpha$ is established above and is not at issue; convexity is a strictly stronger property and the Jensen step of Theorem 1 needs the stronger one. The reader should treat Theorem 1, Proposition 1 and Theorem 3 as resting on a premise the paper has not discharged.
