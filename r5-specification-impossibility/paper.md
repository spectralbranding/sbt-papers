...
Organizational design frameworks prescribe configurations across interdependent levels, yet none quantifies the limits of exhaustive specification. This paper applies high-dimensional geometry to OrgSchema Theory's (OST) 8x6 activation matrix, formalized as a 48-dimensional specification space $[0,1]^{48}$. Three results follow. The Coverage Impossibility Theorem: at resolution $\varepsilon = 0.1$ there are $10^{48}$ distinguishable specifications, each covering a ball of volume $V_{48}(0.1) \approx 1.38 \times 10^{-60}$, so even $10^{20}$ specifications cover $10^{-40}$ of the space. The Effective Dimensionality Theorem: the cascade model reduces effective dimensionality to $d_{\text{eff}} = 15.75$ at an illustrative $\gamma = 0.5$, a 67% reduction. The Forkability Theorem: the fork model decomposes the space into shared and private subspaces, formalizing franchises and open-source ecosystems. An information-theoretic analysis sets 159.4 bits of full specification against a working-memory capacity near 23 bits, a ratio that reduces to the dimension count over the chunk count and is resolution-free. These geometric results are complementary to Simon's bounded-rationality argument: cognitive limits make optimization difficult, whereas geometric impossibility precludes exhaustive coverage regardless of cognitive capacity. The paper distinguishes specification impossibility from NK landscape search complexity, supplying the first formal volumetric justification for OST's specialization requirement.

**Keywords**: organizational design, specification space, high-dimensional geometry, coverage impossibility, dimensionality reduction, cascade constraints, forkability
...
OrgSchema Theory (OST; Zharnikov [-@zharnikov-2026-organizational-schema-theory-test-driven]) provides the architectural framework for this formalization. OST models organizations as configurable systems with six hierarchical levels — Purpose (L0), Values (L1), Strategy (L2), Structure (L3), Process (L4), and Artifacts (L5) — each specified across eight configuration dimensions that parallel the eight dimensions of Spectral Brand Theory [@zharnikov-2026-spectral-brand-theory-computational-framework]. The resulting $8 \times 6 = 48$ parameters form the organization's *activation matrix*, a complete specification of how the organization is configured at every level. OST posits that higher levels constrain lower levels (the *cascade model*) and that organizations can share higher-level specifications while diverging on lower levels (the *fork model*), but these claims have been justified only qualitatively. (Note: the six-level hierarchy here is distinct from the six-tier acquisition-target transferability ontology of Zharnikov [-@zharnikov-2026-dual-hierarchies-organizational-transferability-six], which concerns structural separability in M&A contexts; a full alignment of the two hierarchies is deferred to future work.)

The present paper provides the geometric foundations. Three existing bodies of work address adjacent impossibility arguments without fully capturing the geometric case. Simon [-@simon-1947-administrative-behavior] established that cognitive limits preclude optimal specification; Williamson [-@williamson-1975-markets-hierarchies-analysis; -@williamson-1985-economic-institutions-capitalism] showed that contractual incompleteness results from bounded rationality and opportunism; Suh [-@suh-1990-principles-design-oxford] demonstrated that information content constrains product design decomposition. The present result is strictly complementary to all three: geometric impossibility obtains regardless of cognitive capacity, contractual framing, or fitness function, and therefore holds on grounds that Simon's, Williamson's, and Suh's arguments do not address. No existing organizational design framework quantifies the volumetric coverage of its parameter space, the gap this paper fills. Drawing on the theory of high-dimensional volumes, three results are proved:

1. **Coverage impossibility.** The 48-dimensional specification space $[0,1]^{48}$ is so vast that any finite collection of organizational specifications covers a negligible fraction of it. At resolution $\varepsilon = 0.1$ per dimension, a single specification covers a ball of volume $V_{48}(0.1) \approx 1.38 \times 10^{-60}$, and even $10^{20}$ specifications cover only $10^{-40}$ of the space. Exhaustive specification is not merely impractical but geometrically impossible; compression through cascade constraints and forkability is the geometric response, and whether it is also cognitively necessary is a behavioural question the paper specifies a test for rather than settles.
...
OrgSchema Theory [@zharnikov-2026-organizational-schema-theory-test-driven] models an organization as a configurable system specified across six hierarchical levels and eight configuration dimensions. The six-level hierarchy captures the progression from identity (purpose, values) through direction (strategy) and configuration (structure, process) to instantiation (artifacts); this structure is broadly consistent with Galbraith's [-@galbraith-2014-designing-organizations-strategy] Star Model (strategy, structure, processes, rewards, people) and Mintzberg's [-@mintzberg-1979-structuring-organizations-synthesis] design parameters, extended to make the top and bottom of the hierarchy explicit and to impose a strict cascade ordering. The eight configuration dimensions are adopted as the modeling primitive because they span the semiotic, narrative, ideological, experiential, social, economic, cultural, and temporal axes along which observers reliably distinguish organizations. The six levels are:
...
Each level is specified across eight configuration dimensions that parallel the eight dimensions of Spectral Brand Theory [@zharnikov-2026-spectral-brand-theory-computational-framework]: Semiotic, Narrative, Ideological, Experiential, Social, Economic, Cultural, and Temporal. This parallel structure is not coincidental: OST posits that an organization's internal configuration determines its brand emissions, with each level contributing signals across all eight dimensions.

The 8x6 matrix of parameter values forms the organization's **activation matrix** $A \in [0,1]^{8 \times 6}$, where $A_{ij} \in [0,1]$ represents the activation level of dimension $i$ at organizational level $j$. When vectorized, $A$ becomes a point $a = \text{vec}(A) \in [0,1]^{48}$ in the 48-dimensional specification space. The activation matrix is designed to support construct-level discipline in organizational assessment, addressing concerns about questionable measurement practices [@flake-2020-measurement-schmeasurement-questionable] by requiring explicit parameter specification before measurement; the matrix's pre-specification requirement imposes the kind of discipline that Putnick and Bornstein [-@putnick-2016-measurement-invariance-conventions] identify as necessary for cross-organization comparability.
...
The central geometric phenomenon driving the results is the well-known but underappreciated *volume collapse* of the unit ball relative to the unit cube in high dimensions [@vershynin-2018-highdimensional-probability-introduction]. The volume of the unit ball in $\mathbb{R}^n$ is:
...
This is not a pathology of high-dimensional geometry but its defining feature. The cube $[0,1]^{48}$ has $2^{48} \approx 2.81 \times 10^{14}$ corners, and the volume concentrates near them because each dimension independently pushes mass toward the extremes. A "moderate" or "balanced" specification — one near the center of the cube — is volumetrically rare. This has immediate consequences for organizational design: the typical configuration is extreme in most of its 48 parameters, not balanced across them. An analogous concentration analysis on the probability simplex appears in Zharnikov [-@zharnikov-2026-cohort-boundaries-high-dimensional-perception]; the volumetric concentration in $[0,1]^{48}$ shares the same underlying mechanism.
...
The choice of $\varepsilon = 0.1$ is motivated by practical considerations. Can an organizational designer meaningfully distinguish between a "0.3" and a "0.4" on a given parameter — say, the degree of formalization in decision-making processes? In most organizational contexts, a 10-point scale per dimension represents the outer limit of reliable discrimination. Note that the $\varepsilon = 0.1$ resolution claim requires that OST parameters be metric rather than ordinal; if parameters are ordinal, the volume-based argument requires additional justification. Liddell and Kruschke [-@liddell-2018-analyzing-ordinal-data] document the conditions under which metric-model analyses of ordinal data yield valid inferences; OST's continuous $[0,1]$ parameterization is designed to satisfy those conditions, and empirical validation of the metric interpretation remains an open question for future research.
...
*Applicability condition, not a falsification test.* Theorem 1 is a volume computation on $[0,1]^{48}$ and no observation can contradict it; a version of this paper stated instead that the theorem is "falsified if the OST parameter space is demonstrated to have intrinsic dimensionality substantially below 48 (e.g., fewer than 20 effective dimensions)", and that clause is withdrawn for two reasons. The first is that it names an applicability condition, not a falsifier: at a lower dimension $d$ the same arithmetic still holds, with $V_d(0.1)$ in place of $V_{48}(0.1)$. The second is that this paper's own Theorem 2 already meets it — $d_{\text{eff}} = 15.75$ at $\gamma = 0.5$ — so as written the paper would state an impossibility whose own model satisfies its stated falsifier.

*Reconciling Theorem 1 with Theorem 2.* The two numbers are different quantities. Theorem 1 counts the parameters a designer must place a value on: the cascade does not delete any of the 48 activation parameters, it makes 32.25 of them deterministic functions of the others, so a specification is still a point in $[0,1]^{48}$. Theorem 2's $d_{\text{eff}}$ counts the *free* parameters that remain after an assumed cascade. And 15.75 is not a measurement — it is what the closed form returns at an illustrative $\gamma = 0.5$, as the Boundary Conditions subsection below states.

That distinction resolves the contradiction but does not make the coincidence harmless, so the consequences are worth printing. Applied at $d = 15.75$, $V_{15.75}(0.1) \approx 10^{-16.32}$: covering 1% of the space would take $2.1 \times 10^{14}$ specifications rather than the $7.3 \times 10^{57}$ of Corollary 1.1, and the $10^{20}$ templates of part (iii) would cover it many times over. The strong form of the coverage claim — that even $10^{20}$ specifications are negligible — therefore holds at 48 dimensions and fails at 15.75. What survives at both is the claim against realistic template counts: $10^{10}$ specifications, the plain-language figure below, cover $10^{-49.86}$ of the 48-dimensional space and still only $10^{-6.32}$ of a 15.75-dimensional one.

*What would genuinely bear on the theorem.* The applicability question is the intrinsic dimensionality of the empirical distribution of real organizational specifications, measured rather than assumed — by an intrinsic-dimension estimator on measured activation matrices, not by substituting a chosen $\gamma$ into Theorem 2. The threshold depends on how many templates one is willing to entertain, and it can be stated exactly: for the coverage to fall below $10^{-3}$ requires $d \geq 9.5$ at $10^6$ templates, $d \geq 13.0$ at $10^{10}$, and $d \geq 21.1$ at $10^{20}$. A measured intrinsic dimensionality below the relevant threshold would leave Theorem 1's arithmetic intact and its organizational conclusion unsupported.*
...
The same arithmetic, viewed across orders of magnitude in template count, makes the impossibility result immune to optimistic accounting (Table 2):
...
The severity of coverage impossibility depends critically on dimensionality. To illustrate, coverage properties are compared across different dimensional frameworks (Table 3):
...
**Lemma 1 (Cascade Per-Level Dimensions).** *At cascade coupling $\gamma = 0.5$, the effective free dimensions per level are as reported in Table 4:*
...
The effective dimensionality varies substantially with $\gamma$ (Table 5):
...
Anchor values from the closed form: $R(.1) \approx .22$, $R(.3) \approx .51$, $R(.5) \approx .67$, $R(.7) \approx .76$, $R(.9) \approx .82$ (computed by the companion script's proposition1_gamma_prediction function). The prediction is sharper than the qualitative loose-versus-tight coupling distinction [@weick-1976-educational-organizations-as]: it specifies the functional form, not just the direction, of the relation between coupling and coherence.

*The correlation clause is circular as it stands, and is withdrawn.* An earlier version of this paper stated that Proposition 1 is falsified if the paired sample $\{(\hat{\gamma}_o, \hat{R}_o)\}$ exhibits a Pearson correlation $r < .50$ with $R(\hat{\gamma})$. The only estimator of $d_{\text{eff}}$ supplied anywhere in this paper is Theorem 2's closed form in $\gamma$. If $\hat{R}_o$ is computed as $1 - d_{\text{eff}}(\hat{\gamma}_o)/48$, then $\hat{R}_o = R(\hat{\gamma}_o)$ identically and $r = 1$ for every conceivable sample, including a sample of pure noise. The test cannot fail, so it is not a test.

*What the proposition needs to be testable.* $\hat{R}$ must be constructed from an estimate of $d_{\text{eff}}$ that does not pass through $\gamma$. Two routes are available on measured $48$-parameter activation matrices pooled across organizations. The first is a linear one: the participation ratio $(\sum_i \lambda_i)^2 / \sum_i \lambda_i^2$ of the eigenvalues of the parameter covariance matrix, which returns an effective dimension without reference to any cascade. The second is nonlinear: a manifold intrinsic-dimension estimator applied to the same matrices. Either yields a $\hat{d}_{\text{eff}}$ that can disagree with $8(1-(1-\hat{\gamma})^6)/\hat{\gamma}$, and the disagreement is the empirical content.

*Falsification, restated: Proposition 1 is falsified if $\hat{R}_o = 1 - \hat{d}_{\text{eff},o}/48$, computed by an estimator independent of $\hat{\gamma}$, exhibits a Pearson correlation below .50 with $R(\hat{\gamma}_o)$ across organizations, or if the residuals $\hat{R}_o - R(\hat{\gamma}_o)$ display systematic dependence on level — for example, if the $\gamma$ implied separately at each adjacent-level pair differs by more than .20 within the same organization, indicating that the uniform-$\gamma$ assumption fails. The second clause is testable as it stands and does not depend on the first, because per-pair $\gamma$ estimates are separate measurements rather than one closed form evaluated twice.*
...
*Cascade strength as a modeling parameter.* The coupling parameter $\gamma \in (0,1]$ is a modeling assumption, not a derived constant. At $\gamma = 1$, the cascade collapses to perfect determinism: all five lower levels are fully determined by L0 (Purpose), leaving only 8 free dimensions. At $\gamma \to 0$, the cascade dissolves into full independence and all 48 dimensions are free — the model degenerates to the unconstrained baseline of Theorem 1. Empirical estimation of $\gamma$ from cross-organizational panel data is therefore essential before quantitative claims (e.g., $d_{\text{eff}} = 15.75$) are treated as descriptive rather than illustrative; the estimation methodology is discussed below, under Methodological Implications for Empirical Organizational Design Research.

*Dimensionality is OST-specific; the volume argument is not.* The $8 \times 6 = 48$ dimensional structure is a feature of OST's activation matrix. The volume-collapse argument, however, generalizes to any framework whose dimension exceeds the threshold derived after Theorem 1, which depends on the template count entertained: $d \geq 13.0$ against $10^{10}$ templates and $d \geq 21.1$ against $10^{20}$, for coverage below $10^{-3}$. At $d \gtrsim 15$ the ball-to-cube ratio has already fallen below $10^{-6}$ (see Table 1). The impossibility is not specific to OST; OST happens to instantiate it with a theoretically motivated dimension count.

*Uniform-prior scope.* The coverage bounds in Theorem 1 and Corollary 1.1 assume a uniform distribution over $[0,1]^{48}$ — equivalently, that every point in the specification space is equally relevant. If real organizational specifications cluster on a low-dimensional subspace (a viability manifold), the coverage bound applies to that subspace and $V_{48}(0.1)$ would be replaced by $V_{d_{\text{sub}}}(0.1)$ for the subspace's intrinsic dimension $d_{\text{sub}}$. The impossibility persists as long as $d_{\text{sub}}$ clears the template-count threshold derived after Theorem 1 — $d_{\text{sub}} \geq 13.0$ against $10^{10}$ templates, $d_{\text{sub}} \geq 21.1$ against $10^{20}$ — which the diversity of real organizational forms makes plausible at the lower figure; empirical TDA-based estimation of $d_{\text{sub}}$ remains an open question.
...
The cascade model reduces effective dimensionality within a single organization. The fork model addresses a different question: how can multiple organizations share part of a specification while diverging on the rest? OST's fork model introduced in the Preliminaries posits that organizations can share higher-level specifications (L0 through some level $k-1$) while independently specifying lower levels ($k$ through L5). This is the structure of franchises (shared L0--L2, divergent L3--L5), open-source projects (shared L0--L2, divergent L3--L5), and denominational organizations (shared L0--L1, divergent L2--L5). Felin and Zenger [-@felin-2014-closed-or-open] frame the analogous open-versus-closed innovation choice as a governance decision over which problem-solving subspaces to share with external participants; the fork model captures the structural counterpart of that decision in the activation-matrix framework.
...
**Fork depth and organizational variety.** The fork depth $k$ determines the balance between uniformity and variety (Table 6):
...
**Plain-language interpretation.** A franchise that specifies purpose, values, and strategy (L0--L2) and has moderate cascade coupling ($\gamma = 0.5$) effectively constrains its franchisees to about 1.75 dimensions of genuine freedom in structure, process, and artifacts. This is why well-run franchises feel similar even when they differ in details: the cascade ensures that the "details" are largely determined by the shared higher levels. Table 7 reports the full interaction surface across cascade strengths and fork depths.
...
Miller's [-@miller-1956-magical-number-seven] finding that human working memory can hold approximately $7 \pm 2$ chunks has been updated by Cowan [-@cowan-2001-magical-number-4], who revised the estimate to $4 \pm 1$ for pure working memory capacity. Even using Miller's more generous estimate, if each "chunk" carries $\log_2(10) = 3.32$ bits (one dimension at resolution 0.1), then working memory can hold:

$$H_{\text{WM}} \approx 7 \cdot 3.32 = 23.2 \text{ bits}$$

The ratio $H_{\text{WM}} / H_{48} = 23.2 / 159.4 = 14.6\%$ means that a human designer can hold approximately one-seventh of a full organizational specification in working memory at any given time. Under Cowan's [-@cowan-2001-magical-number-4] revised estimate of 4 chunks, the ratio falls to 8.3%.

**What this ratio is, stated plainly.** It is a derivation, and the resolution plays no part in it. Both terms carry the same per-dimension factor $\log_2(1/\varepsilon)$, so

$$\frac{H_{\text{WM}}}{H_{48}} = \frac{c \cdot \log_2(1/\varepsilon)}{nL \cdot \log_2(1/\varepsilon)} = \frac{c}{nL},$$

for a chunk capacity of $c$ dimensions: $7/48 = .146$ under Miller and $4/48 = .083$ under Cowan, for any $\varepsilon$ whatsoever. Choosing a finer resolution inflates both bit counts identically and leaves the ratio where it was. The bit arithmetic is therefore a restatement of the dimension count, not an independent measurement of cognitive load, and the substantive content of the comparison is the assumption that one chunk equals one activation dimension.

While Simon [-@simon-1947-administrative-behavior] established that cognitive limits preclude optimal specification, the present result is strictly complementary: geometric impossibility obtains even for a cognitively unbounded agent, since no finite collection of specifications can cover more than a negligible fraction of $[0,1]^{48}$, on grounds that are independent of cognitive capacity. Even biases in judgment under uncertainty [@tversky-1974-judgment-under-uncertainty] that reduce effective specification quality below the theoretical bound reinforce rather than constitute the impossibility: the geometric argument holds independently of how well individuals reason.

**Corollary 3 (Compression and Working Memory).** *Under the identification of one working-memory chunk with one activation dimension, a full 48-dimensional specification exceeds a $7 \pm 2$-chunk capacity by a factor of $48/7 = 6.9$, and a $4 \pm 1$-chunk capacity by $48/4 = 12$. The comparison is resolution-free: it is the dimension count set against the chunk count.*

*What the corollary does not establish.* That a specification exceeds working-memory capacity does not by itself make cascade or fork compression *cognitively necessary*, because designers do not hold specifications in working memory. They work with external representations — documents, diagrams, schemas — and the literature on distributed cognition gives no reason to expect an unaided-recall bound to transfer to an externally scaffolded task. The cognitive-necessity claim is a behavioural claim and requires a behavioural test.

*The test that would carry it.* Assign designers the same 48-parameter specification task under two conditions — with a cascade scaffold that fixes L0 first and derives lower levels from it, and without one, both with full access to external representations — and score the resulting specifications for completeness and internal consistency by a rubric fixed in advance. Cognitive necessity predicts that unscaffolded designers cannot produce complete and consistent specifications at rates approaching the scaffolded condition, and that the gap widens with the number of levels. Until such a comparison is run, the corollary should be read as what its arithmetic supports: a dimension-count-to-chunk-count ratio, with the necessity claim marked as untested.

*Falsification: the corollary's arithmetic is not falsifiable, being a ratio of two stated constants. Its two substantive commitments are. The chunk identification fails if the number of independently varying activation parameters at L0 — the one level the cascade leaves free at any $\gamma$ — is measured to be 6 or fewer across organizations, since $H_{\text{eff}}$ would then fall to at most $6 \times 3.322 = 19.9$ bits, below the 23.2-bit Miller capacity, and no compression would be needed to bring a specification within reach. The necessity claim fails if the scaffolded and unscaffolded conditions of the behavioural test above produce specifications of indistinguishable completeness and consistency, or if organizations are observed to produce coherent full-dimensional specifications with no cascade or fork structure at all.*

An earlier version of this paper offered a different first clause — that the corollary is falsified if "the effective information content of a full organizational specification is substantially below 23 bits" — and that clause is withdrawn because it is unreachable under the paper's own model. Level L0 contributes 8 free dimensions at every $\gamma$, so $d_{\text{eff}} \geq 8$ and $H_{\text{eff}} \geq 8 \times 3.322 = 26.6$ bits, above the 23.2-bit capacity by construction. A condition the model forbids carries no falsifying content, which is why the replacement above targets the L0 dimension count instead: that quantity is measurable and is not fixed by the model.
...
This is still more than double working memory capacity, but the cascade imposes a sequential structure: the designer specifies L0 (8 dimensions = 26.6 bits), then uses it to derive L1 (4 free dimensions = 13.3 bits), and so forth. At each level, the information load is within or near working memory capacity (Table 8):
...
*Notes*: WM capacity = 23.2 bits [@miller-1956-magical-number-seven, 7-chunk estimate]; 13.3 bits under Cowan [-@cowan-2001-magical-number-4] 4-chunk estimate. Under either estimate, L1--L5 are within WM capacity at $\gamma = 0.5$; L0 is marginal under Miller and borderline under Cowan. Information per level = free dims $\times \log_2(10)$. Reproduced by the companion computation script (table_information_load function).

The cascade transforms a 159.4-bit simultaneous specification problem into a sequential one requiring at most 26.6 bits at any step. Whether that sequencing is what makes hierarchical organizational design — starting from purpose and deriving lower levels — workable in practice, rather than a philosophical preference, is what the behavioural comparison specified under Corollary 3 would decide.
...
- Organizational architecture (hierarchy, delegation, incentives) affects the efficiency of search [@rivkin-2003-balancing-search-stability].

The NK framework is fundamentally about *navigation*: given a fitness landscape, how does an organization find good configurations through local search? The central question is computational: how hard is it to find the optimum? Ashby's [-@ashby-1956-an-introduction-to] Law of Requisite Variety offers a complementary cybernetic prior: a controller must match the state-space complexity of the system it controls. Ashby's law constrains the *controller* state space; the present paper constrains the *specification* state space — the two are complementary but distinct.

### The Present Paper: Specification Coverage

The present paper addresses a fundamentally different question: *how much of the specification space can any collection of configurations cover?* This question has several structural differences from the NK question (Table 9):
...
*Notes*: NK = Kauffman [-@kauffman-1993-origins-order-selforganization] as applied by Levinthal [-@levinthal-1997-adaptation-rugged-landscapes] and Rivkin [-@rivkin-2000-imitation-complex-strategies]. Present paper's geometric results are independent of fitness function specification. No numerical computations in this table.
...
quadrantChart
    ...
    R5 OST this paper: [0.75, 0.75]
    Suh Axiomatic Design: [0.25, 0.75]
    Kauffman NK Landscapes: [0.25, 0.25]
    Levinthal Fitness Landscapes: [0.75, 0.25]
```
...
The coverage impossibility theorem provides a formal explanation for a widely observed but poorly explained phenomenon: the persistent failure of "best practices" approaches to organizational design. Every decade produces new management frameworks claiming to identify the optimal way to organize — scientific management, total quality management, lean, agile, holacracy — and every decade the previous framework's universality is questioned. Ambiguity in organizational life [@march-1976-ambiguity-choice-organizations] is not a temporary state awaiting better frameworks but a structural feature of organizations operating in incompletely specified design spaces; the geometric explanation is simple: each framework represents a small neighborhood in $[0,1]^{48}$, and no collection of neighborhoods can cover more than a negligible fraction of the space.
...
When a consulting firm applies a template to an organizational design engagement, it implicitly projects the 48-dimensional specification space onto a low-dimensional subspace defined by the template's parameters. A framework with 5 key dimensions (e.g., strategy, structure, systems, shared values, skills) projects $[0,1]^{48}$ onto $[0,1]^5$, discarding 43 dimensions of organizational specification. This projection is formally an instance of the spectral metamerism construct [@zharnikov-2026-spectral-metamerism-brand-perception-projection]: distinct 48-dimensional organizational configurations that produce identical 5-dimensional template projections are organizationally metameric [@zharnikov-2026-organizational-metamerism-when-distinct-configurations] — indistinguishable by the template but genuinely different.
...
2. **Organizational design must be generative, not enumerative.** Rather than selecting from a catalog of organizational types (an enumerative approach that the coverage impossibility theorem shows cannot be comprehensive), organizations must generate their specifications from first principles, using the cascade model to make the generation process cognitively tractable. Near-transfer (sharing the higher-level subspace) and far-transfer (diverging subspaces) in organizational learning [@argote-1999-organizational-learning-creating] correspond directly to the fork model's shared and private subspaces: knowledge that transfers easily maps to the shared subspace; knowledge that requires adaptation maps to the private subspace.
...
4. **Forking is geometrically efficient.** The fork model allows organizations to share the cognitively expensive higher-level specification (purpose, values, strategy = 24 dimensions = 79.7 bits) while independently exploring the lower-level implementation space. This is why franchise systems, open-source projects, and denominational structures have emerged independently across domains: they represent a geometrically efficient response to the specification impossibility problem. Satisficing behavior [@cyert-1963-behavioral-theory-firm] is the behavioral consequence of the same impossibility: organizations search for specifications that are "good enough" precisely because exhaustive search across $[0,1]^{48}$ is geometrically precluded.

### Output Specification Versus Coordination Specification

The 48-dimensional activation matrix is not uniform. OST's six levels divide into two structurally different specification types that differ in their susceptibility to AI compression. The formal projection of organizational specification onto observable brand emissions in SBT's 8-dimensional space is developed in Zharnikov [-@zharnikov-2026-brand-as-modular-layer-tiered].
...
*Coordination specification* (L3--L5: Structure, Process, Artifacts) defines *how to do* the work — roles, reporting relationships, approval flows, workflow sequencing, and the physical and digital infrastructure that supports coordination. Mintzberg [-@mintzberg-1979-structuring-organizations-synthesis] established that standardization of outputs (L0--L2) and standardization of work processes (L3--L5) are alternative coordination mechanisms, with the latter derivable from the former when process steps can be fully specified from output criteria. This insight is formalizable in quantitative terms: at moderate coupling ($\gamma = 0.5$), the output specification layers (L0--L2) contribute 14 of the 15.75 effective dimensions; the coordination specification layers (L3--L5) contribute only 1.75. The asymmetry is not accidental: coordination dimensions are derivative of output dimensions, so their effective freedom collapses precisely because the cascade propagates output-level constraints downward. The cascade model formalizes the implication that Mintzberg's [-@mintzberg-1979-structuring-organizations-synthesis] analysis carried: when output specification is strong, coordination specification is nearly determined. High-$\gamma$ cascade propagation corresponds to Mintzberg's standardization of outputs; low-$\gamma$ loose coupling corresponds to mutual adjustment.
...
*Static analysis.* The paper analyzes the specification space at a single point in time. Real organizations evolve, and their activation matrices change as they adapt to environments, learn from experience, and respond to external pressures. A dynamic extension would model organizational evolution as trajectories in $[0,1]^{48}$, connecting to the diffusion-on-manifolds framework developed in Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-diffusion]. Observer-relative measurement at L1 introduces additional variability beyond geometric projection loss: response-process variability in how organizational parameters are assessed [@schwarz-1999-selfreports-how-questions] means that empirical estimates of activation-matrix values reflect both the true specification and measurement noise, a limitation that the static analysis does not capture.
...
**Connection to Spectral Brand Theory.** OST's activation matrix determines an organization's brand emissions in SBT's 8-dimensional perception space. The mapping from $[0,1]^{48}$ (organizational specification) to $\mathbb{R}^8_+$ (brand emissions) is a 48-to-8 projection whose properties (null space dimension, information loss, metamerism) could be analyzed using the tools developed in Zharnikov [-@zharnikov-2026-spectral-metamerism-brand-perception-projection] for the 8-to-1 projection. A future paper mapping the formal relationship between the present paper's L0--L5 levels and the six-tier acquisition-target ontology of Zharnikov [-@zharnikov-2026-dual-hierarchies-organizational-transferability-six] would clarify when and how the two hierarchies align or diverge.
...
*Estimating $\gamma$ from panel data.* The cascade coupling parameter $\gamma$ is the paper's central empirical unknown. A natural estimation strategy uses a hierarchical mixed-effects model: at each organizational level $i$, the variance of level-$i$ specifications conditional on level $i-1$ provides an estimate of the residual freedom $(1-\gamma)$, and $\gamma$ is recovered as one minus that conditional variance ratio. Cross-organizational or cross-period panel designs — comparing specification variation at L3 (Structure) conditional on shared vs. divergent L2 (Strategy) across a sample of firms — can provide identification. Measurement invariance [@putnick-2016-measurement-invariance-conventions] should be verified before pooling parameter estimates across organizations.

*Operationalizing the activation matrix.* The $8 \times 6$ matrix is operationalizable through the PRISM-Org instrument and existing OST measurement scales [@zharnikov-2026-organizational-schema-theory-test-driven], which provide structured elicitation items for each configuration dimension at each hierarchical level. Researchers should report both the raw $8 \times 6$ matrix and the derived effective-dimensionality estimate $\hat{d}_{\text{eff}}$ for each organization, enabling cross-study aggregation and replication. The metric interpretation of parameters should be validated with standard tests before computing volume-based bounds.
...
The script includes assertion checks that compare each cited figure against the value the script computes, to stated precision. Those assertions catch transcription drift between the paper and the script; because every result here is a closed form evaluated in both places, they are not an independent verification that the closed forms are the right ones. One rounding note: $H_{48} = 48 \times \log_2(10) = 159.45$ bits, which the paper rounds to 159.4, and $H_{\text{eff}} = 15.75 \times \log_2(10) = 52.3$ bits. The script documents the exact values in comments.
