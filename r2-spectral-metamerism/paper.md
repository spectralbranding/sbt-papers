...
**Keywords**: spectral metamerism, Johnson-Lindenstrauss lemma, dimensionality reduction, brand perception, null space, information loss
...
We adopt the notation and definitions of Zharnikov [-@zharnikov-2026-spectral-brand-theory-computational-framework; -@zharnikov-2026-brand-space-geometry-formal-metric]. A brand's *emission profile* (spectral profile) is a vector $s = (s_1, \ldots, s_8) \in \mathbb{R}^8_+$ representing signal strength across the eight SBT dimensions (Table 1):
...
### The L1/L2 Distinction as Dimensionality Reduction
...
For the case-study data from Zharnikov [-@zharnikov-2026-spectral-brand-theory-computational-framework], we use the canonical emission profiles established in Zharnikov [-@zharnikov-2026-brand-space-geometry-formal-metric], reproduced in Table 2:
...
## Metamerism in Brand Perception
...
$$\phi(s_A) = \phi(s_B) \quad \text{and} \quad d_{\mathcal{B}}(s_A, s_B) > 0 \tag{1}$$
...
$$|\phi(s_A) - \phi(s_B)| < \epsilon \quad \text{and} \quad d_{\mathcal{B}}(s_A, s_B) > \delta \tag{2}$$
...
In brand perception under SBT, the parallel is exact (Table 3):
...
```mermaid
graph TD
    S1["Semiotic<br/>s1"] --> PHI["phi: R8+ to R1<br/>scalar grade"]
    S2["Narrative<br/>s2"] --> PHI
    S3["Ideological<br/>s3"] --> PHI
    S4["Experiential<br/>s4"] --> PHI
    S5["Social<br/>s5"] --> PHI
    S6["Economic<br/>s6"] --> PHI
    S7["Cultural<br/>s7"] --> PHI
    S8["Temporal<br/>s8"] --> PHI
    PHI --> G["Grade<br/>A+  A-  B+  B-  C-"]
    PHI --> NS["Null space ker(phi)<br/>dim 7<br/>invisible differences"]
    NS --> MA["Metameric profiles<br/>s and s prime<br/>phi(s) = phi(s prime)<br/>d(s, s prime) > 0"]
```
...
|--------|-----------------|-------------|--------------------|-----------------------------|---------------------|
| Typical DR (PCA / MDS) | $\leq 1 - k/n = 87.5\%$ variance lost | $k \geq 2$ | Top-$k$ principal components; global variance | Residual $n - k$ null-space dimensions remain invisible to any $k$-dimensional summary | Perceptual maps; brand positioning plots |
...
$$k \geq \frac{C \ln N}{\epsilon^2} \tag{4}$$
...
$$\epsilon \geq \sqrt{\frac{\ln N}{k}} \tag{7}$$
...
$$\epsilon \geq \sqrt{\ln N} \tag{8}$$

*In particular, Table 5 reports the required distortion:*
...
*For $\epsilon = 0.3$ (30% distortion tolerance), Table 6 reports the minimum dimensions:*
...
*For $\epsilon = 0.1$ (10% distortion tolerance), Table 7 reports the corresponding requirements:*
...
**Corollary 1** (The 5-level grade as geometric inevitability). *SBT's 5-level grade (A+, A-, B+, B-, C-) is a 1D projection that necessarily loses most of the 8D structure. The loss is not a design flaw but a geometric consequence of compressing $\mathbb{R}^8$ to $\mathbb{R}^1$. No alternative grade scale (7-point, 10-point, continuous) fundamentally resolves the problem; the null space remains 7-dimensional regardless of the output scale's granularity. This result also fixes what is scale-dependent and what is not. The retention ratio is scale-dependent: $\rho = \log_2 L / (nB)$ grows without bound in $L$, so the 11.6% figure of the Grade Information Capacity subsection is the retention of the five-point scale specifically and is not a ceiling on scalar grades. The null space is not: it remains 7-dimensional at every $L$, so no refinement of the output scale recovers any of the seven suppressed directions.*

*Proof.* The null space dimension is $n - k = 8 - 1 = 7$ for any mapping from $\mathbb{R}^8$ to $\mathbb{R}^1$, independent of the discretization of the output. The retention ratio, by contrast, is a function of $L$ alone once $n$ and $B$ are fixed: $\rho(L) = \log_2 L / (nB)$. At $L = 5$ it is $2.32/20 = 11.6\%$; at $L = 100$ it is $6.64/20 = 33.2\%$; it reaches unity at $L = 2^{nB} = 2^{20} \approx 10^6$, the point at which the one-dimensional output is a serial number enumerating the profile rather than a summary of it. No finite $L$ short of that enumeration alters the kernel: for every $L$, a *linear* map $\mathbb{R}^8 \to \mathbb{R}^1$ has a 7-dimensional null space and cannot be injective. The information the grade omits is therefore a consequence of the projection, not of its granularity. $\square$
...
$$M_s = \{s' \in \mathbb{R}^8_+ : \phi(s') = \phi(s)\} \tag{10}$$
...
### Observer-Dependent Null Spaces
...
$$\ker(\phi_w) = \{ \delta \in \mathbb{R}^8 : \langle w, \delta \rangle = 0 \} \tag{11}$$
...
*Applicability condition, not a falsification test*: Proposition 1 is a fact about hyperplanes in $\mathbb{R}^8$. Two non-proportional normals span a two-dimensional subspace, so their orthogonal complements meet in dimension 6, and no observation can contradict that. A study returning identical metameric classifications for two Fisher-Rao-separated observers would show that those observers were not evaluating through distinct linear functionals; it would be a measurement result, not a counterexample. Earlier versions of this paper offered exactly that study as a falsification condition, and it is withdrawn. What is genuinely at risk is the *applicability* of the proposition, and that is what should be tested: the substantive commitment is that an observer's evaluation is a linear functional $\langle w, \text{clr}(s) \rangle$ of the profile with a stable $w \in \Delta^7$. It fails if elicited evaluations from a single observer across $\geq 20$ brand profiles admit no fixed $w$ within measurement error — if, for example, the residuals from the best-fitting linear functional are systematically structured by brand category, or the fitted $w$ shifts with the comparison set. Such a result would leave the hyperplane geometry intact and the paper's use of it unsupported, because there would be no one null space per observer for two observers to intersect.
...
**Empirical illustration on the canonical case set, and what it cannot show.** PCA on the five canonical brand profiles tabulated above[^canonical-pca] yields PC1 = 54.9% of total variance, PC1+PC2 = 80.2%, and PC1+PC2+PC3 = 92.3%. These shares must not be read against the 12.5% figure of Theorem 4. Five points in $\mathbb{R}^8$ span an affine subspace of dimension at most four, so the sample covariance matrix has at most four nonzero eigenvalues whatever the population covariance is. PC1 therefore exceeds 25% by construction and the leading three components exhaust three quarters of a rank-four spectrum, before any inter-dimensional correlation exists to be detected. Theorem 4's 12.5% is a population quantity, and a sample of five cannot speak to it in either direction.

The comparison that is available is against a rank-limited null, and the companion script computes two. Under the parametric null of Theorem 4 — five draws with $\Sigma = \sigma^2 I_8$, so that the dimensions are independent with equal variance — the median PC1 share is 50.9% and the median three-component share is 94.8%; the observed 54.9% sits 4.0 percentage points above the null median at $p = .314$, and the observed 92.3% sits 2.5 percentage points *below* it at $p = .768$ (100,000 draws). Under a distribution-free permutation null that resamples the case set itself, shuffling each dimension independently across the five brands so that every marginal is preserved and only cross-dimensional structure is destroyed, the medians are 52.2% and 95.0%, giving $p = .363$ and $p = .815$ (100,000 permutations).

Neither null is rejected on either statistic. The honest reading of the case set is therefore that it supplies no evidence of inter-dimensional correlation: its spectrum is what five arbitrary points in $\mathbb{R}^8$ produce. Whether real brand profiles are in fact correlated — whether heritage, design language and cultural resonance co-vary, or price positioning anti-correlates with luxury signalling — is an open empirical question that a sample with $N \gg 8$ would have to settle. What is unaffected is the theoretical claim: $1/8$ is the worst case under independence, correlation can only improve on it, and the metameric collapse argument applies to whatever structure the leading component ignores wherever the true spectrum lies.
...
**Memoryless-channel caveat.** Theorem 5 treats the $L$-level grade as a memoryless discrete channel.[^memoryless] In sequential brand evaluation — where successive grade utterances by a single observer over time are not independent (anchoring, order effects, learning) — the memoryless assumption breaks down, and $C_{\text{grade}} = \log_2 L$ becomes an upper bound on the per-evaluation rate rather than the total-experiment rate. A formal treatment of the sequential setting would require modeling the inter-evaluation dependence structure; the bound established here is the most permissive case and therefore strengthens, not weakens, the information-deficit conclusion when sequential dependence is present.

[^memoryless]: A memoryless channel produces output $Y_t$ depending only on input $X_t$, with successive uses statistically independent. Sequential brand evaluation by a single observer typically violates this assumption.
...
Three independent trials with different random seeds are reported in Table 8:
...
*Notes*: Brand profiles drawn from log-normal distribution ($\mu = 0.5$, $\sigma = 0.5$) on $\mathbb{R}^8_+$. Metameric threshold: $d_{8D} > 1.0$ and $d_{1D} < 0.3$ under a uniformly random projection direction. Sensitivity of the fraction to both thresholds is reported in Table 9.
...
The metameric fraction is defined by two chosen constants, and earlier versions of this paper asserted without evidence that it was "robust to threshold variation within $\pm 20\%$". Table 9 reports the grid that assertion should have carried. Each cell is the mean metameric fraction over 2,000 independent trials of the design above at the stated thresholds.
...
The corrected statement is therefore narrower than the one it replaces. The metameric fraction is insensitive to where "well separated in 8D" is drawn and roughly linear in where "indistinguishable in 1D" is drawn; the qualitative conclusion that a substantial minority — between roughly a quarter and two fifths of pairs — collapses under a single random projection survives the whole $\pm 20\%$ grid, but the specific 31--39% band is a property of the published thresholds and not a threshold-free result.

### Companion Computation Script

Every computed value in this paper is reproduced by `R2_R3_computations.py`, published in the paper's public code mirror (https://github.com/spectralbranding/sbt-papers/tree/main/r2-spectral-metamerism/code) and run with:

```
uv run --with numpy python R2_R3_computations.py
```

The script fixes seed 42 and emits, in order: the Table 8 metameric-fraction trials and the stable-band summary; the Table 9 threshold grid; the rank-limited PCA nulls for the canonical case set (Variance Retention subsection); and the brand-space resampling null for Aitchison audit distances (Brand audits become vector comparisons, in the Vectorized Alternative subsection). Its assertions compare the paper's printed values against the values the script computes, so they catch transcription drift between paper and script; they are not an independent check of whether the model is the right one.
...
**Brand audits become vector comparisons.** Instead of subjective "does this feel on-brand?" judgments, compute $d_{\mathcal{B}}(s_{\text{intended}}, s_{\text{observed}})$ using the Aitchison metric and compare it to a null constructed in the same space. Earlier versions of this paper took that null from the concentration result in Zharnikov [-@zharnikov-2026-brand-space-geometry-formal-metric], where the expected squared distance between random points on $\Delta^7$ is $7/36$, and used $\sqrt{7/36} = .441$ as the threshold. That transfer is invalid and is withdrawn here. The quantity $7/36$ is a Dirichlet moment on the *observer* simplex, whose coordinates are weights summing to one; brand profiles live in $\mathbb{R}^8_+$ and are compared by the Aitchison metric on log-ratios, a different space on a different scale. Every distance in the case-study set — .88 for Hermès-Patagonia through 1.76 for Hermès-Tesla — exceeds .441, so the threshold classifies every comparison ever made as misaligned and carries no discriminating content.

The null has to be resampled in brand space. Drawing each dimension independently from its observed marginal across the five canonical profiles, which preserves the per-dimension spread and destroys only the joint structure, yields a null Aitchison distance with median 1.15 and a 5th-to-95th percentile band of .54 to 1.77 (200,000 draws, companion script). Against that null, .441 falls at the 2nd percentile, while the case-study distances span the 26th to the 95th: the five canonical brands are no further apart than independently assembled profiles with the same dimension-wise variability, their median distance of 1.23 stands against a null median of 1.15, a gap of .08 on a null whose standard deviation is .38. An operational audit threshold should be the upper tail of such a null computed on the auditing organization's own profile population, not a constant imported from observer space.
...
3. **Communication to non-specialist audiences.** A board-level report may require a scalar grade for executive communication, provided the audience understands it is a lossy summary. The 11.6% retention of a five-point grade should be disclosed as a caveat, together with the fact that a finer scale raises retention only logarithmically and leaves the 7-dimensional null space untouched.
...
## Connection to MDS and Survey Design
...
A machine reader produces the same collapse without a questionnaire, which removes the obvious rival explanation for it. Chu and Hou [-@chu-2026-incumbent-advantage-brand-bias] had three commercial large language models rank products; where the options were specification-identical, the models fell back on brand name and returned the known incumbent in 100% of 670 trials — distinct products rendered indistinguishable by the reading instrument, which is a maximal metameric outcome. The collapse is not a fixed property of the reader: supplying a distinguishing signal reverses it, with product parameters taking 82.4% of ranking variance against 1.2% for brand identity in a three-way analysis of variance (*N* = 14,395), and a rating difference of .075 flipping half of head-to-head comparisons. What that case contributes is the elimination of an alternative account. Metameric collapse observed in a survey instrument can always be attributed to questionnaire construction, respondent fatigue or scale anchoring; a machine reader has none of those, so what remains is the projection itself. Two limits of the source are worth stating: it is a preprint whose retrieval-augmentation probe is explicitly directional, so that result is not relied on here, and its "authority language" manipulation is built from fabricated clinical-evidence claims — the measured mechanism is what is cited, not the tactic.

The measurement-instrument implications go further. Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-why] establishes that brand perception is non-ergodic: repeated measures of the same observer over time produce a time-average that diverges from the cross-sectional population average. A scalar tracking dashboard, by averaging across observers as well as collapsing across dimensions, compounds two independent sources of information loss — the dimensional projection analyzed here and the ergodicity failure analyzed in Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-why]. The combined effect is a tracking output whose fidelity to the underlying multi-dimensional, longitudinal perceptual reality is doubly bounded: by the 11.6% information retention of a five-point grade established in the Grade Information Capacity subsection and by the non-ergodicity gap documented in Zharnikov [-@zharnikov-2026-non-ergodic-brand-perception-why].
...
### Foundational-to-Specialization Map
...
|-------|---------|-------------------|------------------|-----------|
| 2026e (R2, this paper) | Foundational brand perception | 8D spectral profile $s \in \mathbb{R}^8_+$ | Distinct profiles with identical scalar grades | Null space dim 7; 31--39% metameric pairs (Monte Carlo); 11.6% info retention |
| 2026v (R15, LLM-specific) | LLM-mediated brand perception | 8D profile through LLM attention head | Brand profiles collapsed by LLM representational bottleneck | Dimensional collapse: LLMs project to fewer effective dimensions than human observers |
| 2026x (R16, behavioral) | Behavioral AI specification | Agent behavioral profile | Distinct specifications producing identical behavioral outputs | Behavioral metamerism: different agent identities indistinguishable by output alone |
...
The null-space analysis in the Null Space Analysis section is stated for Euclidean distances on raw profiles. Under the Aitchison metric (which operates on centered log-ratio transforms), the null space of a linear projection $\phi(\text{clr}(s)) = \mathbf{u}^T \text{clr}(s)$ is the set:

$$\ker(\phi \circ \text{clr}) = \{s \in \mathbb{R}^8_+ : \mathbf{u}^T \text{clr}(s) = c\} \tag{18}$$
...
A pair $(s_i, s_j)$ is classified as metameric if $d_{8D}(s_i, s_j) > 1.0$ (substantial separation in 8D) and $d_{1D}(s_i, s_j) < 0.3$ (near-indistinguishable in 1D). The thresholds are chosen to identify cases of clear 8D separation that are collapsed by the projection. Their influence is not symmetric: across a $\pm 20\%$ grid on both (Table 9) the fraction moves 3.0 percentage points with the 8D threshold and 12.0 points with the 1D threshold, spanning 23.8% to 38.5% overall.

### A.4 Summary of Numerical Results

For reference, Table 11 collects the key computed values used throughout the paper:

**Table 11.** Summary of Key Computed Values Cited Throughout the Paper.

| Quantity | Value | Section |
|----------|-------|---------|
| Null space dimension ($\mathbb{R}^8 \to \mathbb{R}^1$) | 7 | Dimension of the Null Space |
| Variance retained (1D, independent dims) | 12.5% | Variance Retention |
| Required distortion: $N=10$ | $\epsilon \geq 1.52$ (152%) | Application to 8D-to-1D Projection |
| Required distortion: $N=50$ | $\epsilon \geq 1.98$ (198%) | Application to 8D-to-1D Projection |
| Grade information capacity (5-point) | 2.32 bits | Grade Information Capacity |
| Full profile information (8D) | $\sim 20$ bits | Grade Information Capacity |
| Information retention ratio (5-point grade) | 11.6% | Grade Information Capacity |
| Information retention ratio ($L$-level grade) | $\log_2 L / 20$ | Grade Information Capacity |
| Canonical case set: PC1 share | 54.9% ($p = .314$ vs rank-limited null) | Variance Retention |
| Canonical case set: PC1+PC2+PC3 share | 92.3% ($p = .768$ vs rank-limited null) | Variance Retention |
| Metameric fraction across the $\pm 20\%$ threshold grid | 23.8--38.5% | Threshold Sensitivity |
| Brand-space null Aitchison distance (median, 5th--95th) | 1.15, .54--1.77 | The Vectorized Alternative |
| Metameric pair frequency (Monte Carlo) | 31--39% | Monte Carlo Verification |
| JL minimum $k$ for $\epsilon=0.3$, $N=10$ | $\geq 256$ | Minimum Faithful Dimension |
| JL minimum $k$ for $\epsilon=0.3$, $N=50$ | $\geq 435$ | Minimum Faithful Dimension |

*Notes*: All distortion values computed with conservative constant $C = 1$; see Theorem 1 notes and Appendix A.1 for the Larsen-Nelson lower bound context. The two case-set $p$-values are one-sided Monte Carlo tail probabilities against the parametric rank-limited null (100,000 draws); the distribution-free permutation null gives $p = .363$ and $p = .815$ respectively. Cross-reference the Johnson-Lindenstrauss Bounds, Null Space Analysis, Information-Theoretic Bounds, and Monte Carlo Verification sections for derivations.
