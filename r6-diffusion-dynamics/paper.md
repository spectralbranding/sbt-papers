...
Working Paper v2.2.0 – March 2026 (revised August 2026)

## Abstract

Brand perception is inherently dynamic, yet formal continuous-time manifold models of it remain scarce. This paper models an observer's brand perception as a stochastic process on $S^7_+$, the positive octant of the 7-sphere in Spectral Brand Theory's eight-dimensional perception space. Signal encounters drive Brownian motion on the manifold, signal decay introduces drift toward a neutral prior, and negative conviction creates absorbing boundaries. A Stratonovich SDE on $S^7_+$ yields four results: well-posedness up to absorption (Theorem 1); exponential survival decay at rate $\lambda_{D,1} \sigma_0^2/2$, $\lambda_{D,1} = 112$ (Theorem 2); Dirichlet spectral gap 48, giving faster mixing to the quasi-stationary distribution than on the full sphere (Theorem 3); and time-ensemble divergence with probability 1, establishing non-ergodicity through absorption rather than Peters' [-@peters-2019-ergodicity-problem-economics] multiplicative framing (Theorem 4). Five brands are ordered by boundary distance; matching that order to SBT coherence grading requires drift strengths read off the grades, so Proposition 7 states a mechanism, not a demonstrated link. Recovery time scales as $1/\delta^2$ in residual perception (Proposition 5). The D/A survival bound sweeps $[.31, .72]$ across the plausible parameter range, making the Goldilocks zone $r^* \in [.55, .65]$ a calibration (Proposition 6). Results formalize the dynamic distinction between vectorized and rasterized brand management.

**Keywords**: stochastic differential equations on manifolds, non-ergodicity, brand perception dynamics, absorbing boundaries, Laplace-Beltrami operator, mixing time

---

Every brand theory acknowledges that brand perception changes over time. Keller's [-@keller-1993-conceptualizing-measuring-managing] Customer-Based Brand Equity model invokes "brand building" and "brand leveraging" as temporal processes. Aaker [-@aaker-1991-managing-brand-equity] discusses brand loyalty as something earned over repeated interactions. Kapferer [-@kapferer-2008-new-strategic-brand, 4th ed.] describes the "brand identity prism" as evolving through market engagement. Yet none of these frameworks provides a formal dynamical model — a mathematical description of *how* perception evolves, *what drives* its evolution, and *why* some trajectories are reversible while others are not.

The gap is not merely aesthetic. Without a dynamical model, brand theory cannot answer several questions that arise naturally from Spectral Brand Theory's [@zharnikov-2026-spectral-brand-theory-computational-framework] static framework:
...
2. **Why do time averages and ensemble averages diverge?** Peters [-@peters-2019-ergodicity-problem-economics] demonstrated that the failure of ergodicity in economic systems has profound consequences for decision theory. SBT's ergodicity coefficient $\varepsilon$ [@zharnikov-2026-spectral-brand-theory-computational-framework] claims an analogous non-ergodicity in brand perception, but the claim has remained qualitative. *When exactly* do time and ensemble averages diverge, and *by how much*?
...
The gap in formal dynamical modeling is not for lack of adjacent work. Longitudinal brand tracking studies (e.g., Young & Rubicam's BrandAsset Valuator, Millward Brown's BrandZ) measure brand perception at multiple time points, but these are sequences of static snapshots rather than dynamical systems — they do not specify equations of motion or characterize trajectories. Brand loyalty dynamics models [@dick-1994-customer-loyalty-toward] formalize the relationship between relative attitude and patronage but operate on one-dimensional attitude scales, not on a multi-dimensional metric space; the geometry of loyalty trajectories is undefined. Attitude-change models in the persuasion literature, including the Elaboration Likelihood Model [@petty-1986-communication-persuasion-central], specify conditions under which attitude change occurs and identify central versus peripheral processing routes, but they neither produce a metric on attitude space nor model attitude evolution as a stochastic process with drift and diffusion components. The common deficiency is structural: none of these frameworks places brand perception in a formal metric space, and without a metric space, drift, diffusion, and absorbing boundaries — the essential objects of a dynamical model — cannot be defined.

The most proximate antecedents in quantitative marketing are Bass [-@bass-1969-new-product-growth] and Mahajan, Muller, & Bass [-@mahajan-1990-new-product-diffusion], whose diffusion models describe population-level adoption as an ODE over calendar time. These models share the label "diffusion" but operate on a fundamentally different object: aggregate adoption counts in a scalar time series, with no observer geometry, no state space, and no absorbing boundaries. R6 models individual-observer trajectories on a Riemannian manifold; the connection to Bass-style diffusion is terminological rather than structural. Sriram, Balachander, & Kalwani [-@sriram-2007-monitoring-dynamics-brand] introduced a scalar AR(1) state-space model for brand equity dynamics using scanner data — the most direct precursor for the idea that brand equity has a time-varying latent state. R6 generalizes that scalar state to the full 8-dimensional SBT perception space with Riemannian geometry and absorbing boundaries, recovering Sriram et al.'s [-@sriram-2007-monitoring-dynamics-brand] dynamics as the low-dimensional, no-boundary limit. Netzer, Lattin, & Srinivasan [-@netzer-2008-hidden-markov-model] modeled individual-level customer relationship dynamics using a hidden Markov model (HMM) on finite discrete states. Their approach is the closest prior art in marketing science: it tracks individual trajectories over time and acknowledges that customers can enter and exit states. R6 differs structurally in three ways: the state space is continuous and metric ($S^7_+$ rather than a finite lattice), the boundary conditions are absorbing rather than absorbing-and-resurrecting (their model permits return from all states), and the non-ergodicity result follows from the absorbing boundary geometry rather than from a mixing-time argument on a finite Markov chain. The Netzer et al. [-@netzer-2008-hidden-markov-model] HMM cannot represent the Tylenol-BP divergence in recovery time scaling (Proposition 5), because that divergence arises from the $1/\delta^2$ singularity as $\delta \to 0$ — a continuous-space phenomenon invisible to finite-state models. Table 1 summarizes how these traditions differ along the structural features that matter for the present model.
...
|---|---|---|---|---|
| Bass [-@bass-1969-new-product-growth] diffusion | Scalar adoption count | Deterministic ODE | No | None |
| Netzer, Lattin, and Srinivasan [-@netzer-2008-hidden-markov-model] HMM | Finite discrete states | Stochastic | No | Absorbing and resurrecting |
| Sriram, Balachander, and Kalwani [-@sriram-2007-monitoring-dynamics-brand] AR(1) | Scalar latent equity | Stochastic | No | None |
| Aydogdu et al. [-@aydogdu-2017-opinion-dynamics-general] manifold opinion dynamics | Compact Riemannian manifold | Stochastic | Yes | None (no boundary) |
| R6 (this paper) | Positive 7-octant $S^7_+$ | Stochastic SDE | Yes | Absorbing Dirichlet at octant boundary |

*Notes*: HMM = hidden Markov model. "Multidimensional" means the model tracks more than one perception or equity dimension simultaneously. "Boundary handling" refers to irreversible state transitions. R6 recovers the Sriram et al. [-@sriram-2007-monitoring-dynamics-brand] scalar dynamics as the low-dimensional, no-boundary limit.

The approach connects three previously separate intellectual traditions. From **stochastic analysis on manifolds** [@hsu-2002-stochastic-analysis-manifolds; @stroock-2000-introduction-analysis-paths], we draw the formulation of Brownian motion on Riemannian manifolds and the connection between the Laplace-Beltrami spectrum and mixing times. From **ergodicity economics** [@peters-2019-ergodicity-problem-economics; @peters-2016-evaluating-gambles-using], we draw the distinction between time and ensemble averages and the consequences of their divergence. From **opinion dynamics** [@hegselmann-2002-opinion-dynamics-bounded; @deffuant-2000-mixing-beliefs-among], we draw the insight that belief evolution can be modeled as a geometric process — though existing opinion dynamics models operate in flat Euclidean space, not on curved manifolds.

The paper builds on the metric framework established in Zharnikov [-@zharnikov-2026-brand-space-geometry-formal-metric], which defined the Aitchison [-@aitchison-1986-statistical-analysis-compositional] compositional-data metric on brand signal space $\mathbb{R}^8_+$ and the Fisher-Rao metric on observer weight space $\Delta^7$. Where that paper established the *geometry* of brand perception (how to measure distances), this paper establishes the *dynamics* (how positions change over time). The two together — statics and dynamics — provide a complete mathematical foundation for SBT.

**Contributions.** This paper makes three contributions. First, a Stratonovich SDE on $S^7_+$ is developed with absorbing Dirichlet boundaries, yielding a closed-form survival probability (Theorem 2), exact Dirichlet eigenvalue $\lambda_{D,1} = 112$ (Section Absorbed Brownian Motion on $S^7_+$), and the non-ergodicity result (Theorem 4). Second, this paper closes a gap that Sriram et al. [-@sriram-2007-monitoring-dynamics-brand] and Netzer et al. [-@netzer-2008-hidden-markov-model] leave open: no prior brand-dynamics model combines a multi-dimensional metric space, Riemannian geometry, absorbing boundaries, and conditional non-ergodicity simultaneously; the SDE on $S^7_+$ does. Third, the model provides a dynamical foundation for vectorized brand management: it proves that ensemble surveys misrepresent individual time-averaged experience (Theorem 4), formalizes the absorption-exploration trade-off that the D/A Goldilocks zone is calibrated within (Proposition 6), and proposes coherence-conditional drift as the mechanism that would connect coherence type to absorption risk ordering (Proposition 7). The applied companion papers R9 [@zharnikov-2026-non-ergodic-brand-perception-why] and R12 [@zharnikov-2026-coherence-type-as-crisis-predictor] extend these results to cross-sectional tracking and crisis prediction respectively; the present paper establishes the formal foundation both draw on.
...
Figure 1 lays out the dependency structure of the four theorems and three propositions for readers who prefer to navigate the paper by results rather than sections. Theorem 1 (well-posedness) underwrites everything that follows; Theorem 2 (survival probability) supplies the eigenvalue $\lambda_{D,1} = 112$ that propagates into Theorem 3 (mixing) and Theorem 4 (non-ergodicity); Proposition 5 ($1/\delta^2$ recovery) and Proposition 7 (absorption ordering) are calibrated against Theorem 2; Proposition 6 (the D/A trade-off) draws on the drift-diffusion balance set up in the Effect of Drift on Absorption section.
...
Spectral Brand Theory [@zharnikov-2026-spectral-brand-theory-computational-framework] models a brand as a stellar object emitting signals across eight typed dimensions (Table 2):
...
*Notes*: Dimensions follow the canonical SBT ordering [@zharnikov-2026-spectral-brand-theory-computational-framework]. Index matches component position in emission profile $s = (s_1, \ldots, s_8)$ and observer spectral profile $w = (w_1, \ldots, w_8)$.
...
SBT assigns five coherence levels based on the emission profile analysis (Table 3):
...
*Notes*: Grades and types from Zharnikov [-@zharnikov-2026-spectral-brand-theory-computational-framework]. "Ecosystem coherence" denotes omnidirectional signal reinforcement; "incoherent" denotes contradictory signals across dimensions.

The canonical emission profiles from Zharnikov [-@zharnikov-2026-brand-space-geometry-formal-metric] are:
...
*Notes*: Profiles are canonical SBT values from Zharnikov [-@zharnikov-2026-spectral-brand-theory-computational-framework; -@zharnikov-2026-brand-space-geometry-formal-metric]. Scale: 1–10 signal intensity per dimension. Used as input to the SDE analysis throughout this paper.
...
$$\text{Vol}(S^7_+) = \frac{1}{2^8} \text{Vol}(S^7) = \frac{\pi^4}{768} \approx .1269$$

as established in Zharnikov [-@zharnikov-2026-brand-space-geometry-formal-metric], reflecting the $1/256$ compression factor from restricting to non-negative coordinates.
...
where $\tilde{f}$ is the degree-zero homogeneous extension of $f$ [@berger-1971-le-spectre-dune].
...
where $P_x = I - x x^T$ is the orthogonal projection onto the tangent space $T_x S^{n-1}$ at $x$, and $\circ$ denotes the Stratonovich differential [@hsu-2002-stochastic-analysis-manifolds; @stroock-2000-introduction-analysis-paths]. The Stratonovich formulation is natural here because it preserves the manifold constraint: if $B_0 \in S^{n-1}$, then $B_t \in S^{n-1}$ for all $t \geq 0$.
...
The **mixing time** $\tau_{\text{mix}}$ is the time required for the distribution of $B_t$ to become approximately uniform, regardless of the initial condition. Following standard definitions [@levin-2009-markov-chains-mixing], we define mixing time via total variation distance:
...
$$\tau_{\text{mix}}(S^7) \asymp \frac{2}{7} \approx .286$$
...
Existing opinion dynamics models [@hegselmann-2002-opinion-dynamics-bounded; @deffuant-2000-mixing-beliefs-among] formulate belief evolution as random walks in flat Euclidean space $\mathbb{R}^n$. The key differences when moving to spherical geometry are:
...
2. **Curvature**: The positive curvature of $S^7$ accelerates mixing relative to flat space. The Lichnerowicz theorem [@lichnerowicz-1958-geometrie-des-groupes] gives $\lambda_1 \geq \frac{n-1}{n-2} K(n-2) = n - 1$ for an $(n-1)$-dimensional manifold with Ricci curvature $\text{Ric} \geq (n-2) K$. For $S^{n-1}$ with $K = 1$, this recovers $\lambda_1 \geq n - 1$, which is tight.
...
4. **Boundary effects**: The positive octant $S^7_+$ introduces boundaries that trap the process, fundamentally altering the dynamics. This has no analogue in standard opinion dynamics models (though see Aydogdu et al. [-@aydogdu-2017-opinion-dynamics-general], for opinion dynamics on manifolds without boundary), which operate on unbounded Euclidean domains.
...
2. **Mixing time to the quasi-stationary distribution (QSD)**: The QSD $\pi_{\text{QSD}}$ is the long-run distribution of the process *conditioned on survival*. It exists for absorbed diffusions on compact domains [@cattiaux-2009-quasistationary-distributions-diffusion] and satisfies:
...
where $\phi_1$ is the first Dirichlet eigenfunction (Cattiaux et al. [-@cattiaux-2009-quasistationary-distributions-diffusion], Definition 1.2 / Theorem 1.1).
...
**The numerical value, and what it does not depend on.** At the paper's working noise level $\sigma_0 = .1$, Theorem 3 gives $\tau_{\text{mix}}(S^7_+) \asymp 2/(48 \times .01) = 4.17$ years, against $2/(7 \times .01) = 28.6$ years on the full sphere. Two things follow and are used later. First, the expression contains $\sigma_0$ and the spectral gap and nothing else: no emission profile, no coherence grade, no drift parameter appears in it. Theorem 3 therefore returns the same mixing time for every brand, and any per-brand variation in mixing time requires a quantity Theorem 3 does not contain. Second, adding drift can only accelerate the approach to the conditioned equilibrium relative to pure diffusion, so 4.17 years is the model's slow case at this noise level and the mixing time on the octant is in no circumstance larger than on the full sphere.

### The Ergodicity Coefficient

SBT's ergodicity coefficient $\varepsilon$ measures the reliability of ensemble averages as proxies for individual time averages (Zharnikov [-@zharnikov-2026-spectral-brand-theory-computational-framework], where the concept was introduced qualitatively). We now provide a formal definition grounded in the mixing time analysis.
...
When $\varepsilon \gg 1$ (the observation window is much longer than the mixing time), the process has mixed thoroughly, and ensemble averages are reliable proxies for time averages. When $\varepsilon \ll 1$ (the observation window is short relative to mixing), the process has not mixed, and time averages diverge from ensemble averages. The comparison between the full sphere and the octant runs the other way from what intuition suggests. Theorem 3 gives $\tau_{\text{mix}}(S^7_+) \asymp 2/(48\sigma_0^2)$ against $\tau_{\text{mix}}(S^7) \asymp 2/(7\sigma_0^2)$, so the absorbing octant mixes *faster* and its $\varepsilon$ is the *larger* of the two. What makes the absorbed process non-ergodic is not slow mixing but absorption itself: the conditioned process equilibrates quickly, and the unconditioned process leaves the state space altogether. The ergodicity coefficient measures the first of these and Theorem 4 the second, and the two must not be run together.
...
**What the derivation uses, and what it does not.** The proof turns on two facts and no others: $f$ is bounded, and $\tau$ is finite almost surely. Given those, the numerator of $\bar{f}_T$ freezes and the $1/T$ factor sends the time average to zero. Nothing in the argument depends on the state space being the positive octant of $S^7$ rather than any other domain, on the diffusion being Brownian rather than any other process with almost-sure absorption, on the value $\lambda_{D,1} = 112$, or on the quasi-stationary distribution. The same conclusion holds for absorbed motion on an interval. Theorem 4 is stated here in the geometric setting for continuity with the rest of the paper, but it is a general property of absorbed processes and should not be read as a result about $S^7_+$.

The spectral and quasi-stationary apparatus earns its place elsewhere, and it is worth being explicit about where, because the apparatus is doing real work that Theorem 4 does not display. It supplies the *rate*: $\lambda_{D,1} = 112$ fixes the exponential decay of the survival probability through Theorem 2, and therefore the horizon over which the divergence between time and ensemble averages becomes large enough to matter for a real tracking study — a divergence that is certain in the limit is of no practical interest if it takes a century to appear. It supplies the *conditional* statement: the quasi-stationary distribution gives the non-zero limit on the surviving branch, which is what the corollary below turns into a graded claim about brands rather than an all-or-nothing one. And it supplies the comparison with the unabsorbed sphere. The bare limit in Theorem 4 is qualitative; the apparatus is what makes the non-ergodicity quantitative.
...
At $r = 0$ (all ambient), the process is pure diffusion and absorption is certain (see the Absorption Probability as a Function of Position section). At $r = 1$ (all designed), the process is deterministic and follows the drift toward $s/\|s\|$ — no absorption, but also no "organic" perception development. The SBT-optimal range $r \in [.55, .65]$ corresponds to a regime where:
...
This provides a dynamical rationale for the shape of SBT's D/A Goldilocks zone: too little design risks absorption; too much prevents the stochastic exploration that crystallizes perception. Where the two bounds fall is a separate question, taken up later, and the model does not pin it down. This separation is an architectural simplification: in practice designed campaigns can have stochastic reception (adding diffusion) and ambient trends can be directional (adding drift). The model treats the designed-vs-ambient decomposition as a useful first approximation.
...
The ratio $\alpha \lambda_{\text{enc}} / \kappa$ determines the **effective signal maintenance**: values much greater than 1 indicate a brand that reinforces perception faster than it decays. The signal decay drift $\mu_{\text{decay}}$ defines a manifold-adapted Ornstein-Uhlenbeck process [@uhlenbeck-1930-theory-brownian-motion], and generalizes the Nerlove-Arrow [-@nerlove-1962-optimal-advertising-policy] goodwill decay model to the Riemannian manifold $S^7_+$; where Nerlove and Arrow modeled scalar advertising goodwill decaying toward zero, the drift here pulls toward the neutral prior $x^*$ on $S^7_+$. SBT's temporal compounding mechanism (dimension 8) provides a natural defense against decay: brands with high temporal scores benefit from slow decay rates (heritage and longevity are resistant to memory erosion), creating a self-reinforcing cycle where temporal compounding reduces $\kappa$, which improves signal maintenance, which further strengthens temporal compounding.
...
By the standard formula for expected hitting times of one-dimensional diffusions [@collet-2013-quasistationary-distributions-markov, Ch. 3], the expected time to reach $\epsilon$ starting from $\delta$ is:
...
#### Illustration: Tylenol vs. BP

The semi-permeable boundary model distinguishes between brand crises with different recovery profiles. The two cases below are an illustration of that distinction and not a test of it, and the reason is worth stating before they are read. Neither $\delta$ nor $\epsilon$ is measured for either brand. Both are assigned qualitatively, and they are assigned from recovery outcomes that were already known when the assignment was made: Tylenol recovered, so its residual perception is described as large and its reserves deep; BP did not, so its residual is described as small and its reserves shallow. Parameters fitted to the outcome cannot then predict it. What follows shows what the $1/\delta^2$ scaling would look like in two familiar cases, which is a legitimate expository purpose and the only one claimed.
...
The contrast illustrates the model's structure: **recovery from brand crisis depends not on the severity of the crisis alone, but on the product of crisis depth ($1/\delta^2$) and pre-crisis perceptual reserves ($\epsilon$)** — brands with deep reserves across dimensions survive severe crises because the reflecting drift engages strongly, while brands with shallow reserves in the affected dimension face protracted damage even from moderate crises. The specific timelines (approximately 2 years for Tylenol; incomplete rehabilitation for BP after more than fifteen years) are consistent with Coombs' [-@coombs-2007-protecting-organization-reputations] crisis communication literature and BP's documented public record.

Turning this into evidence requires measuring $\delta$ rather than assigning it. The pre-crisis and post-crisis dimensional scores for both brands are in principle recoverable — from contemporaneous tracking data, from archival survey series, or from a retrospective spectral read of period sources — and $\epsilon$ is estimable from the same series as the level at which the reflecting drift is observed to engage. With $\delta$ measured, the $1/\delta^2$ scaling makes a quantitative prediction about the ratio of the two recovery times that the record can contradict. That measurement is not attempted here, and no claim of empirical support for Proposition 5 is made from these two cases.

### The D/A Goldilocks Zone as a Calibration

The D/A Ratio as Drift Control section observed that the D/A ratio $r$ controls the drift-diffusion balance and that SBT's recommended range of $r \in [.55, .65]$ reflects a requirement of sufficient drift to prevent absorption while maintaining sufficient diffusion to enable crystallization. Here we express that trade-off in the SDE parameters and ask how tightly the model constrains the range. The answer, developed below, is that it constrains the shape of the trade-off but not the location of the interval, which remains a calibration.
...
**Proposition 6** (D/A trade-off, and the calibration of its interval). *For the parametrized SDE with $\sigma_0 = .1$ and $\lambda_{D,1} = 112$, the survival lower bound on the designed/ambient ratio is $r_{\min} = 1.12 / (.56 + \alpha \lambda_{\text{enc}} d_\partial)$. Across the paper's stated plausible range $\alpha \lambda_{\text{enc}} \cdot d_\partial \in [1.0, 3.0]$ this sweeps $[.31, .72]$, so the model constrains $r$ only to that width; it does not select an interval within it. Evaluating at the midpoint $\alpha \lambda_{\text{enc}} \cdot d_\partial = 1.5$ gives $r_{\min} = .54$, and pairing it with an assumed crystallization upper bound of $.65$ reproduces the SBT range $r^* \in [.55, .65]$. That reproduction is a calibration of the model to the observed range and not a prediction of it.*
...
Setting $\gamma(r) = 0$ (the boundary between net absorption and net survival) and substituting $\lambda_{D,1} = 112$, $\sigma_0 = .1$:

$$\frac{112 \times .01 \times (2 - r)}{2} = r \cdot \alpha \lambda_{\text{enc}} \cdot d_\partial$$

$$.56(2 - r) = r \cdot \alpha \lambda_{\text{enc}} \cdot d_\partial$$

For the canonical brands, $\alpha \lambda_{\text{enc}} \cdot d_\partial$ is taken to range from approximately 1.0 (Tesla, weak drift, small $d_\partial$) to 3.0 (Hermès, strong drift, moderate $d_\partial$). Two features of that range should be stated before it is used. Hermès and Tesla have nearly identical boundary distances — $d_\partial = .127$ and $.120$ from the case-study table — so the three-fold spread in the product comes almost entirely from $\alpha \lambda_{\text{enc}}$, which is itself read off the coherence grade and has never been measured; the implied values are $\alpha \lambda_{\text{enc}} \approx 23.6$ for Hermès against $\approx 8.3$ for Tesla. And the resulting bound is highly sensitive to the choice: Table 5 shows $r_{\min}$ falling from .72 to .31 across the range. The analysis below uses a representative value of 1.5, which yields $r_{\min} = .54$; the representative value was chosen after the target interval was known, and no independent measurement selects it.
...
*Notes*: Each row gives the minimum D/A ratio at which net survival exceeds net absorption, computed as $r_{\min} = 1.12/(.56 + \alpha \lambda_{\text{enc}} d_\partial)$. The bound is monotone in the drift-strength parameter and spans more than forty points of $r$ across the stated range, so which value of $r_{\min}$ the analysis reports is decided by the choice of drift strength rather than by the model.
...
The upper constraint is intended to come from the crystallization condition: the trajectory must revisit the neighborhood of $s/\|s\|$ from sufficiently diverse angles to reinforce the prior across multiple dimensions, which requires the diffusion to be non-degenerate in all 7 tangent directions. The value at which that fails is stated as $r_{\text{cryst}} \approx .65$. It should be recorded that this figure is asserted rather than derived: no expression for $r_{\text{cryst}}$ is given anywhere in the paper, no computation produces .65, and no sensitivity analysis is run on it as one is on the lower bound. Combining the survival bound $r \geq .54$ at the representative drift strength with the asserted crystallization bound $r \leq .65$, and rounding the lower bound upward:

$$r^* \in [.55, .65]$$

This coincides with SBT's empirically observed D/A range, and the coincidence is not evidence. Both endpoints were fixed by choices made with the target range in view — a free drift-strength parameter evaluated at a midpoint that lands just below .55, and an upper bound asserted at .65 — while the model's own unconstrained answer for the lower bound is the interval $[.31, .72]$, which contains the SBT range but also most of the unit interval. What the derivation establishes is that the absorption-exploration trade-off has the right *shape*: a survival floor that rises as drift weakens, and an exploration ceiling above it. Where the two meet is a calibration, not a prediction, and the proposition supplies no independent corroboration of the SBT interval. $\square$

*Falsification: as stated, Proposition 6 has no falsifying content — testing the interval $[.55, .65]$ against brand data is circular, because the interval was fitted to that data. A test becomes available only when the drift parameters are measured independently: estimate $\alpha \lambda_{\text{enc}}$ and $d_\partial$ for a sample of brands from encounter and emission data rather than from their coherence grades, compute $r_{\min}$ for each, and derive $r_{\text{cryst}}$ from the crystallization condition. Proposition 6 fails if the observed optimal D/A ratios then fall outside the intervals so computed. The empirical identification strategy section sets out what such an estimation would require.*

## Numerical Demonstration

### Case Study: Five Brands

We apply the framework to SBT's five case-study brands using the canonical emission profiles from Zharnikov [-@zharnikov-2026-brand-space-geometry-formal-metric]. For each brand, we compute the normalized emission profile $\hat{s} = s / \|s\|$ and the minimum coordinate $d_\partial(\hat{s}) = \min_i \hat{s}_i$, which determines proximity to the absorbing boundary.
...
Table 7 reports the full per-dimension normalized profiles underlying these boundary distances.
...
*The second ordering is obtained by reordering the first with per-brand drift strengths read off the SBT coherence grade and type. Its near-match to that grading (C- > B- > B+ > A- > A+) is therefore a property of the input, not a finding, and the same holds of the one departure — IKEA (A-) above Patagonia (B+) — which follows from the hand-assigned distinction between identity and signal coherence. The computed content of the proposition is the first ordering.*
...
The adjusted ordering Tesla > Erewhon > IKEA > Patagonia > Hermès aligns with the SBT coherence grades, and the alignment is definitional rather than evidential. The drift strengths that produce it are, as the Table 8 notes state, qualitative readings of $\alpha\lambda_{\text{enc}}$ taken from each brand's coherence type; no drift strength here is measured from encounter data, and none is derived from the emission profile independently of the grade. A reordering driven by the grade will reproduce the grade, and the single departure from it is the hand-supplied identity-versus-signal distinction rather than a discovered inversion. The section should be read accordingly.

What the model does supply independently of the grading is the first ordering: Tesla ($d_\partial = .120$) > Hermès (.127) > Erewhon (.135) > Patagonia (.237) > IKEA (.249), computed from the canonical emission profiles alone. That ordering does *not* match the coherence grades — Hermès, the A+ brand, sits second-riskiest — and the mismatch is the substantive observation. It says that boundary proximity and coherence are distinct properties of a brand, and it is what motivates the drift term in the first place. The framework's contribution here is therefore a mechanism, a route by which coherence type could govern dynamical stability through drift strength opposing boundary approach, together with the measurement that would be needed to test it: an independent estimate of $\alpha\lambda_{\text{enc}}$ per brand from encounter frequency and signal consistency. Until that estimate exists, the connection between coherence type and dynamical stability is proposed, not shown. Zharnikov [-@zharnikov-2026-coherence-type-as-crisis-predictor] extends this absorption risk ordering to a formal crisis-prediction instrument by showing that pre-crisis coherence grade is a sufficient statistic for predicting recovery time within the $1/\delta^2$ scaling of Proposition 5. $\square$

*Falsification: the drift-adjusted ordering inherits the coherence grade and so cannot test it, but the two orderings are separately falsifiable against tracking data. The model's grade-independent prediction is falsified if observed frequency of dimensional collapse does not rank Tesla > Hermès > Erewhon > Patagonia > IKEA, controlling for encounter rate and noise level. The drift mechanism is falsified if, with $\alpha\lambda_{\text{enc}}$ estimated from encounter data rather than assigned from the grade, the reordering it produces does not move Hermès toward the low-risk end.*
...
Using Theorem 2 with the exact $\lambda_{D,1} = 112$ and $\sigma_0 = .1$ (moderate perceptual noise), the characteristic survival time is:

$$\tau_{\text{char}} = \frac{2}{\lambda_{D,1} \sigma_0^2} = \frac{2}{112 \times .01} \approx 1.78$$

In dimensionless units. Under the illustrative assumption $\sigma_0 = .1$, if we calibrate the time unit as one year (assuming the observer encounters the brand approximately weekly), then $\tau_{\text{char}} = 1.78$ years is an illustrative estimate of the characteristic time for an observer to lose a perceptual dimension through random fluctuation alone; the actual value scales as $1/\sigma_0^2$ and will differ across contexts.

The survival probability at various times, for the equal-weight initial condition $x^* = (1/\sqrt{8}, \ldots, 1/\sqrt{8})$, is reported in Table 9:
...
![](figures/r6_survival_curves.png)
...
Definition 1 applied to Theorem 3 at the paper's working noise level gives one number for all five brands. With $\sigma_0 = .1$ and an observation window $\tau_{\text{char}} = 1$ year, $\tau_{\text{mix}} = 2/(48 \times .01) = 4.17$ years and $\varepsilon = 1/4.17 = .24$ — uniformly non-ergodic, with no ordering across brands, because no brand-specific quantity enters the mixing-time expression. That is the model's own prediction and it is reported here as such.

Table 10 reports a different quantity. Its per-brand mixing times are not evaluations of Theorem 3; they are an assumed calibration in which each brand is assigned an effective diffusion $\sigma_{\text{eff}}$ standing in for the combined effect of noise and coherence-conditional drift, with $\tau_{\text{mix}} = 2/(48\sigma_{\text{eff}}^2)$. The $\sigma_{\text{eff}}$ values this implies are recorded in the table so the assumption is visible.

**Table 10.** Assumed Effective Diffusion and Implied Ergodicity Coefficients by Brand ($\tau_{\text{char}} = 1$ Year), Against the Theorem 3 Baseline.

| Brand | Assumed $\sigma_{\text{eff}}$ | $\tau_{\text{mix}}$ (years) | $\varepsilon$ | Reading |
|-------|---------------------|---------------------|------------|----------------|
| Hermès | .456 | .20 | 5.0 | Effectively ergodic |
| Patagonia | .373 | .30 | 3.3 | Moderately ergodic |
| IKEA | .345 | .35 | 2.9 | Moderately ergodic |
| Erewhon | .228 | .80 | 1.3 | Weakly ergodic |
| Tesla | .129 | 2.50 | .4 | Non-ergodic |
| *Theorem 3 baseline, all brands* | .100 | 4.17 | .24 | Non-ergodic |

*Notes*: $\varepsilon = \tau_{\text{char}} / \tau_{\text{mix}}$ from Definition 1, with $\tau_{\text{mix}} = 2/(48\sigma_{\text{eff}}^2)$. The final row evaluates Theorem 3 at the paper's stated $\sigma_0 = .1$; it carries no brand-specific term and is therefore identical for all five brands. The five $\sigma_{\text{eff}}$ values above it are not measured. They are back-solved from mixing times assigned in the order of the SBT coherence grades, and they require Hermès observers to carry roughly four and a half times the effective diffusion of Tesla observers. Per-observer values vary with the observer's current position $X_t$ on $S^7_+$.

Three things follow. The ordering in Table 10 reproduces the coherence grading because the grading is what generated it, so the table cannot be read as evidence that coherent brands are more ergodic. The absolute values are not the model's: at $\sigma_0 = .1$ the model says every one of these brands is non-ergodic on a one-year window, and only the assumed $\sigma_{\text{eff}}$ inflation lifts four of them above $\varepsilon = 1$. And the direction of the assumption deserves stating plainly, because it is not obvious: a *larger* effective diffusion produces a *shorter* mixing time, so representing strong coherence as fast mixing amounts to representing it as high effective noise. Deriving per-brand mixing times properly requires the spectral gap of the drift-augmented generator on $S^7_+$, which this paper does not compute; that computation is the route to replacing the table with a result.

**Interpretation, under the calibration.** If the assumed $\sigma_{\text{eff}}$ ordering is granted, Hermès at $\varepsilon = 5.0$ has ensemble surveys over one year that closely approximate the time average of any individual observer, while Tesla at $\varepsilon = .4$ has surveys that systematically misrepresent it: the survey overestimates positive perception because the entire perception cloud of observers includes those who have not yet been absorbed — the "survivorship bias" of brand health surveys. The qualitative claim that survivorship bias grows as $\varepsilon$ falls follows from Definition 1 and Theorem 4 and does not depend on the calibration. The per-brand assignment of $\varepsilon$ does depend on it, and awaits the drift-augmented spectral computation or direct longitudinal estimation.

This provides a formal basis for SBT's warning that "brand health is not brand power" [@zharnikov-2026-spectral-brand-theory-computational-framework]. Brand health surveys measure ensemble averages; brand power depends on time averages. The two diverge precisely when the ergodicity coefficient is low.

## Discussion

### The Vectorized-Rasterized Distinction Revisited

The dynamical framework established in this paper deepens the distinction between vectorized and rasterized brand management introduced in Zharnikov [-@zharnikov-2026-spectral-metamerism-brand-perception-projection]. In the rasterized approach, a brand manager takes periodic snapshots of brand perception (annual surveys, quarterly trackers) and makes decisions based on these cross-sectional data. In the vectorized approach, the full trajectory of perception — including its velocity, its drift direction, its proximity to absorbing boundaries, and its crystallization state — is tracked and managed.
...
State-space maximum likelihood techniques [@sriram-2007-monitoring-dynamics-brand] and hidden Markov model fitting procedures [@netzer-2008-hidden-markov-model] provide the methodological blueprint, and latent attrition models with marketing activity covariates [@schweidel-2013-incorporating-direct-marketing] extend this tradition to signal-driven state transitions; full adaptation to the spherical geometry of $S^7_+$ requires manifold-valued state-space estimation. Identifying the parameter vector from naturally occurring data will require instruments that shift signal exposure quasi-randomly, such as geographic media market discontinuities or platform algorithm changes.
...
4. **Non-ergodicity (Theorem 4).** For the absorbed process, time averages and ensemble averages diverge with probability 1. This provides a brand-perception realization of time-ensemble divergence via absorbing-boundary geometry, establishing a mechanism that is independent of Peters' [-@peters-2019-ergodicity-problem-economics] multiplicative framing and that extends Molenaar's [-@molenaar-2004-manifesto-psychology-as; -@molenaar-2009-new-personspecific-paradigm] idiographic critique to a specific dynamical system on a Riemannian manifold. The result itself is a specific instance of the general quasi-stationary-distribution literature [@collet-2013-quasistationary-distributions-markov]; the contribution is its derivation in the brand-perception context with concrete parameter values.

5. **Absorption risk is ordered by boundary distance, and coherence enters as a proposed mechanism (Proposition 7).** For the five case-study brands the model computes the ordering Tesla > Hermès > Erewhon > Patagonia > IKEA from the canonical emission profiles, which does not match the SBT coherence grading. The drift-adjusted ordering Tesla > Erewhon > IKEA > Patagonia > Hermès does match it, but is produced by drift strengths read off those grades and so restates them. The substantive proposal is the mechanism: coherent signaling would create effective drift that opposes absorption, incoherent signaling effective diffusion that accelerates it, and testing it requires estimating drift strength independently of the grade.
...
| Mixing time QSD $\tau_{\text{mix}}(S^7_+)$ at $\sigma_0 = .1$ | $2/.48 \approx 4.17$ |
| Survival time $\tau_{\text{char}}$ at $\sigma_0 = .1$ | 1.78 (time units) |
| Survival probability $S(2, x^*)$ at $\sigma_0 = .1$ | .326 |

**Normalized emission profiles and minimum coordinates** were computed as $\hat{s}_i = s_i / \|s\|_2$ for each brand using the canonical emission profiles from Zharnikov [-@zharnikov-2026-brand-space-geometry-formal-metric]; the resulting $d_\partial(\hat{s})$ values are reported in the Case Study: Five Brands table above.

**Ergodicity coefficient estimates** were computed as $\varepsilon = \tau_{\text{char}} / \tau_{\text{mix}}$ with $\tau_{\text{char}} = 1$ year. Theorem 3 evaluated at the paper's $\sigma_0 = .1$ gives $\tau_{\text{mix}} = 4.17$ years and $\varepsilon = .24$ for every brand, since no brand-specific quantity enters the expression. The per-brand values in the ergodicity table are not that quantity: they follow from an assumed per-brand effective diffusion $\sigma_{\text{eff}}$, back-solved as $\sigma_{\text{eff}} = \sqrt{2/(48\,\tau_{\text{mix}})}$ from mixing times assigned in coherence-grade order. They are a calibration, are labelled as such in that table, and are reproduced by the companion script from a literal table rather than computed.

### Companion Computation Script

***Companion Computation Script.*** All numerical values reported in the Numerical Demonstration section and this Appendix (Vol($S^7_+$) $\approx .1269$, $\lambda_{D,1} = 112$, spectral gap = 48, survival probability table, normalized emission profiles, $d_\partial$ values, and ergodicity coefficients) are reproduced by `r6_diffusion_dynamics.py`, available at `https://github.com/spectralbranding/sbt-papers/tree/main/r6-diffusion-dynamics/code/`. Run with `uv run python r6_diffusion_dynamics.py --seed 42` to reproduce all reported values; the script asserts every paper-quoted value at start-up. Those assertions catch paper-versus-script transcription drift and nothing else: they compare the paper's printed numbers against the script's own outputs, so a value the script takes as an input rather than computing — the per-brand mixing times of Table 10 are held in the script as a literal table — will pass the assertion whatever its provenance. Reproduction here means the printed values match the code that produced them; it is not independent verification that the code implements the model correctly, and it does not test any claim the paper makes about brands. Pass `--write-figures` to regenerate Figure 3 (`figures/r6_phase_diagram.png`) and Figure 4 (`figures/r6_survival_curves.png`) under the same drift coefficients used in the body. The script uses Python 3.12 with NumPy (and matplotlib only when `--write-figures` is set); no other dependencies are required.
